...
def find_island_centers(array_2d, threshold):
    ...
    for i in range(1, num_labels + 1):
        ...
        if total_mass > 0:
            ...
            areas.append(
                np.sum(island_mask)
            )  # Area is the count of pixels in the island
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Visualize Continuous Thought Machine Attention"
    )
    parser.add_argument(
        "--actions",
        type=str,
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="checkpoints/imagenet/ctm_clean.pt",
        help="Path to ATM checkpoint",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="tasks/image_classification/analysis/outputs/imagenet_viz",
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Iterations to use during inference.",
    )
    parser.add_argument(
        "--data_indices",
        type=int,
    ...
    parser.add_argument(
        "--N_to_viz", type=int, default=5, help="When not supplying data_indices."
    )
...
if __name__ == "__main__":
    ...
    if args.debug:
        ...
        validation_dataset_centercrop = datasets.CIFAR100(
    ...
    loader = torch.utils.data.DataLoader(
        validation_dataset, batch_size=1, shuffle=False, num_workers=0, drop_last=False
    )
    loader_crop = torch.utils.data.DataLoader(
        validation_dataset_centercrop,
        batch_size=64,
        shuffle=True,
        num_workers=0,
        drop_last=True,
    ...
    if model.iterations != args.inference_iterations:
        print(
            "WARNING: you are setting inference iterations to a value not used during training!"
        )
            ...
            with tqdm(
                ...
                for bi, (inputs, targets) in enumerate(loader):
                    ...
                    if (
                        ...
                        concatenated_predictions_argsorted = np.argsort(
                            concatenated_predictions, 1
                        )[:, ::-1]

                        for topk in [1, 5]:
                            concatenated_predictions_argsorted_topk = (
                                concatenated_predictions_argsorted[:, :topk]
                            )

                            accs_instant, accs_avg, accs_certain = [], [], []
                            accs_avg_logits, accs_weighted_logits = [], []
                            with tqdm(
                                total=(concatenated_predictions.shape[-1]),
                                initial=0,
                                leave=False,
                                position=1,
                                dynamic_ncols=True,
                                ...
                                for stepi in np.arange(
                                    concatenated_predictions.shape[-1]
                                ):
                                    pred_avg = (
                                        softmax(concatenated_predictions, 1)[
                                            :, :, : stepi + 1
                                        ]
                                        .mean(-1)
                                        .argsort(1)[:, -topk:]
                                    ...
                                    pred_certain = (
                                        concatenated_predictions_argsorted_topk[
                                            np.arange(
                                                concatenated_predictions.shape[0]
                                            ),
                                            :,
                                    ...
                                    accs_instant.append(
                                        np.any(
                                            pred_instant
                                            == concatenated_targets[..., np.newaxis],
                                            -1,
                                        ).mean()
                                    )
                                    accs_avg.append(
                            ...
                            fig.savefig(
                                f"{args.output_dir}/accuracy_types_{topk}.png", dpi=200
                            )
                            fig.savefig(
                                f"{args.output_dir}/accuracy_types_{topk}.pdf", dpi=200
                            )
                            plt.close(fig)
                            print(
                        ...
                        for certainty_threshold in [0.5, 0.8, 0.9]:
                            ...
                            with tqdm(
                                ...
                                pbarinner.set_description(
                                    f"Certainty threshold={certainty_threshold}"
                                )
                                for stepi in np.arange(
                                    concatenated_predictions.shape[-1]
                                ):
                                    certainty_here = concatenated_certainties[
                                        :, 1, stepi
                                    ]
                                    certainty_mask = (
                                        certainty_here >= certainty_threshold
                                    )
                                    predictions_here = (
                                        concatenated_predictions_argsorted_topk[
                                            :, :, stepi
                                    ...
                                    is_correct_here = np.any(
                                        predictions_here
                                        == concatenated_targets[..., np.newaxis],
                                        axis=-1,
                                    )
                                    percentage_corrects.append(
                            ...
                            fig.tight_layout(pad=0.1)
                            fig.savefig(
                                f"{args.output_dir}/steps_versus_correct_{certainty_threshold}.png",
                                dpi=200,
                            )
                            fig.savefig(
                                f"{args.output_dir}/steps_versus_correct_{certainty_threshold}.pdf",
                                dpi=200,
                            )
                            plt.close(fig)
                        ...
                        colours = cmap_diverse = plt.get_cmap("rainbow")(
                        ...
                        color_linspace = np.linspace(
                            0, 1, concatenated_predictions.shape[-1]
                        )
                        with tqdm(
                            total=(concatenated_predictions.shape[-1]),
                            initial=0,
                            ...
                            for stepi in np.arange(concatenated_predictions.shape[-1]):
                                ...
                                for pi in range(len(probability_space) - 1):
                                    ...
                                    mask = (
                                        (
                                            (probabilities >= bin_low)
                                            & (probabilities < bin_high)
                                        )
                                        if pi != len(probability_space) - 2
                                        else (
                                            (probabilities >= bin_low)
                                            & (probabilities <= bin_high)
                                        )
                        ...
                        ax.plot(
                            probability_space,
                            np.linspace(0, 1, len(probability_space)),
                            "k--",
                        )
                        ...
                        sm = plt.cm.ScalarMappable(
                            cmap=cmap_calib,
                            norm=plt.Normalize(
                                vmin=0, vmax=concatenated_predictions.shape[-1] - 1
                            ),
                        )
                        ...
                        fig.tight_layout(pad=0.1)
                        fig.savefig(
                            f"{args.output_dir}/imagenet_calibration.png", dpi=200
                        )
                        fig.savefig(
                            f"{args.output_dir}/imagenet_calibration.pdf", dpi=200
                        )
                        plt.close(fig)
    if "videos" in args.actions:
        if not args.data_indices:  # If list is empty
            ...
            data_indices = np.random.choice(
                np.arange(n_samples), size=num_to_sample, replace=replace
            )
        ...
        for di in data_indices:
            ...
            }  # Store (y,x) path points per head
            head_routes[-1] = []
            ...
            for step_i in range(n_steps):
                ...
                data_img_tensor = inputs[
                    0
                ].cpu()  # Get first item in batch, move to CPU
                ...
                certainties_now = (
                    certainties[0, 1, : step_i + 1].detach().cpu().numpy()
                )  # Assuming index 1 holds relevant certainty
                ...
                attention_interp_bilinear = F.interpolate(
                    torch.from_numpy(attention_now)
                    .unsqueeze(0)
                ...
                attn_mean = (attn_mean - attn_mean_min) / (
                    attn_mean_max - attn_mean_min
                )
                centers, areas = find_island_centers(
                    attn_mean.detach().cpu().numpy(), threshold=0.7
                )
                ...
                if centers:  # If islands found
                    ...
                    -1
                ]:  # If no center now, repeat last known center if history exists
                    head_routes[-1].append(head_routes[-1][-1])

                attn_min = (
                    attention_interp_bilinear.view(n_heads, -1)
                ...
                attention_interp_bilinear = (attention_interp_bilinear - attn_min) / (
                ...
                for ii in range(len(certainties_now)):
                    ...
                    ax_cert.axvspan(
                        ii, ii + 1, facecolor=facecolor, edgecolor=None, lw=0, alpha=0.3
                    )
                # Mark the last point
                ax_cert.plot(
                ...
                ax_cert.set_xlim(
                    [0, n_steps]
                )  # Use n_steps for consistent x-axis limit
                ...
                colours = [
                    "g" if idx == true_class_idx else "b" for idx in topk_indices
                ]

                # Plot horizontal bars (inverted range for top-down display)
                ax_prob.barh(
                    np.arange(k)[::-1], topk_probs, color=colours, alpha=1
                )  # Use barh and inverted range
                ...
                for i, name_idx in enumerate(topk_indices):
                    ...
                    fg_color = (
                        "darkgreen" if is_correct else "crimson"
                ...
                # Normalize each head's map to [0, 1]
                attn_min_plot = (
                    attention_interp_plot.view(n_heads, -1)
                    .min(dim=-1, keepdim=True)[0]
                    .unsqueeze(-1)
                )
                attn_max_plot = (
                ...
                attention_interp_plot = (attention_interp_plot - attn_min_plot) / (
                    attn_max_plot - attn_min_plot + 1e-6
                )
                ...
                for head_i in list(range(n_heads)) + [-1]:
                    ...
                    if axname not in axes:
                        continue  # Skip if mosaic doesn't have this head
                    ...
                    if these_route_steps:  # Only plot if path exists
                        ...
                        for i in range(len(these_route_steps) - 1):
                            ...
                            dy = (
                                y_coords_flipped[i + 1] - y_coords_flipped[i]
                            )  # Use flipped y for delta
                            ...
                            ax_overlay.arrow(
                                ...
                                fc=route_colours_step[i],
                                ec=route_colours_step[i],  # Use step color
                                length_includes_head=True,
                            )
            ...
            imageio.mimsave(
                gif_path, frames, fps=15, loop=0
            )  # loop=0 means infinite loop
            save_frames_to_mp4(
                [fm[:, :, ::-1] for fm in frames],
                os.path.join(index_output_dir, f"{str(di)}_viz.mp4"),
                fps=15,
                gop_size=1,
                preset="veryslow",
            )
        ...
        for di in data_indices:
            ...
            for step_i in range(n_steps):
                ...
                if step_i == model.iterations - 1:
                    fig.savefig(
                        os.path.join(index_output_dir, f"frame_{step_i}.png"), dpi=200
                    )
