...
def has_solved_checker(
    x_maze, route, valid_only=True, fault_tolerance=1, exclusions=[]
):
    ...
    for step in route:
        ...
        next_pos_candidate = list(
            current_pos
        )  # Use a list for mutable coordinate calculation
        if step == 0:
    ...
    if (
        target_pos and final_pos == target_pos and mistakes_made <= fault_tolerance
    ):  # Added mistakes_made check here
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze Asynchronous Thought Machine on Maze Tasks"
    )
    parser.add_argument(
        "--actions", type=str, nargs="+", default=["gen"], help="Actions: 'viz', 'gen'"
    )
    parser.add_argument(
        "--device",
        type=int,
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="checkpoints/mazes/ctm_mazeslarge_D=2048_T=75_M=25.pt",
        help="Path to CTM checkpoint",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="tasks/mazes/analysis/outputs",
        help="Directory for analysis outputs",
    )
    parser.add_argument(
        "--dataset_for_viz", type=str, default="large", help="Dataset for 'viz' action"
    )
    parser.add_argument(
        "--dataset_for_gen",
        type=str,
        default="extralarge",
        help="Dataset for 'gen' action",
    )
    parser.add_argument(
        "--batch_size_test",
        type=int,
        default=32,
        help="Batch size for loading test data for 'viz'",
    )
    parser.add_argument(
        "--max_reapplications",
        type=int,
        default=20,
        help="When testing generalisation to extra large mazes",
    )
    parser.add_argument(
        "--legacy_scaling",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Legacy checkpoints scale between 0 and 1, new ones can scale -1 to 1.",
    )
...
def _load_ctm_model(checkpoint_path, device):
    ...
    print(
        f"Loaded state_dict. Missing keys: {load_result.missing_keys}, Unexpected keys: {load_result.unexpected_keys}"
    )
    model.eval()
    return model
...
if __name__ == "__main__":
    ...
    if "gen" in args.actions:
        ...
        testloader = torch.utils.data.DataLoader(
            test_data,
            batch_size=min(len(test_data), 2000),
            shuffle=False,
            num_workers=1,
        )
        ...
        inputs, targets, actual_lengths = (
            inputs[sorted_indices],
            targets[sorted_indices],
            actual_lengths[sorted_indices],
        )
        ...
        for n_tested in range(test_how_many):
            ...
            while not has_solved and re_applications < args.max_reapplications:
                re_applications += 1
                with torch.no_grad():
                    predictions, certainties, _, _, _, attention_tracking = model(
                        current_input_maze, track=True
                    )

                h_feat, w_feat = model.kv_features.shape[-2:]
                attention_tracking = attention_tracking.reshape(
                    attention_tracking.shape[0], -1, h_feat, w_feat
                )

                n_steps_viz = predictions.shape[
                    -1
                ...
                for stepi in range(n_steps_viz):
                    ...
                    if (
                        attention_tracking is not None
                        ...
                        try:
                            ...
                            if attn_resized.max() > attn_resized.min():
                                ...
                                attn_norm[attn_norm < np.percentile(attn_norm, 80)] = (
                                    0.0
                                )
                                frame = np.clip(
                                    (
                                        np.copy(frame)
                                        * (1 - attn_norm[:, :, np.newaxis])
                                        * 1
                                        + (
                            ...
                            Exception
                        ):  # Keep broad except for visualization robustness
                            pass
                    frame_resized = cv2.resize(
                        frame,
                        (
                            int(current_maze_np.shape[1] * 4),
                            int(current_maze_np.shape[0] * 4),
                        ),
                        interpolation=cv2.INTER_NEAREST,
                    )  # Corrected shape[1]*4 for height
                    long_frames.append(
                ...
                chosen_pred_route = (
                    predictions[0, :, where_most_certain]
                    .reshape(-1, 5)
                    .argmax(-1)
                    .detach()
                    .cpu()
                ...
                current_start_loc_list = np.argwhere(
                    (current_maze_np == [1, 0, 0]).all(axis=2)
                ).tolist()

                # Ensure current_start_loc_list is not empty before trying to access its elements
                if not current_start_loc_list:
                    print(
                        f"Warning: Could not find start location in maze {maze_idx_display} during reapplication {re_applications}. Stopping reapplication."
                    )
                    break  # Cannot proceed without a start location

                solved_now, final_pos, _ = has_solved_checker(
                    current_maze_np,
                    chosen_pred_route,
                    True,
                    fault_tolerance,
                ...
                path_img = draw_path(
                    current_maze_np, chosen_pred_route, cmap=cmap, valid_only=True
                )
                if ongoing_solution_img is None:
                    ongoing_solution_img = path_img
                else:
                    mask = (
                        (np.any(ongoing_solution_img != path_img, -1))
                        & (~np.all(path_img == [1, 1, 1], -1))
                        & (~np.all(ongoing_solution_img == [1, 0, 0], -1))
                    )
                ...
                next_input[0, :, old_start_idx[0], old_start_idx[1]] = (
                    1.0  # Reset old start to path
                )

                if (
                    0 <= final_pos[0] < next_input.shape[2]
                    and 0 <= final_pos[1] < next_input.shape[3]
                ):
                    next_input[0, :, final_pos[0], final_pos[1]] = torch.tensor(
                        [1, 0, 0], device=device, dtype=next_input.dtype
                    )  # New start
                else:
                    print(
                        f"Warning: final_pos {final_pos} out of bounds for maze {maze_idx_display}. Stopping reapplication."
                    )
                    break
            ...
            if has_solved:
                ...
                if long_frames:
                    save_frames_to_mp4(
                        [fm[:, :, ::-1] for fm in long_frames],
                        os.path.join(maze_output_dir, f"combined_process.mp4"),
                        fps=45,
                        gop_size=10,
                        preset="veryslow",
                        crf=20,
                ...
                print(
                    f"Failed maze of length {maze_actual_length} after {re_applications} reapplications. Not saving visuals for this maze."
                )

            if maze_actual_length not in results:
                results[maze_actual_length] = []
            ...
            if sorted_lengths:
                success_rates = [
                    np.mean([r[0] for r in results[l]]) * 100 for l in sorted_lengths
                ]
                reapps_mean = [
                    (
                        np.mean([r[1] for r in results[l] if r[0]])
                        if any(r[0] for r in results[l])
                        else np.nan
                    )
                    for l in sorted_lengths
                ]
                ax_success.plot(
                    sorted_lengths, success_rates, linestyle="-", color=palette[0]
                )
                ax_reapp.plot(
                    sorted_lengths, reapps_mean, linestyle="-", color=palette[5]
                )
            ax_success.set_xlabel("Route Length")
            ax_success.set_ylabel("Success (%)")
            ax_reapp.set_xlabel("Route Length")
            ax_reapp.set_ylabel("Re-applications (Avg on Success)")
            fig_success.tight_layout(pad=0.1)
            fig_reapp.tight_layout(pad=0.1)
            fig_success.savefig(
                os.path.join(
                    output_gen_dir, f"{args.dataset_for_gen}-success_rate.png"
                ),
                dpi=200,
            )
            fig_success.savefig(
                os.path.join(
                    output_gen_dir, f"{args.dataset_for_gen}-success_rate.pdf"
                ),
                dpi=200,
            )
            fig_reapp.savefig(
                os.path.join(
                    output_gen_dir, f"{args.dataset_for_gen}-re-applications.png"
                ),
                dpi=200,
            )
            fig_reapp.savefig(
                os.path.join(
                    output_gen_dir, f"{args.dataset_for_gen}-re-applications.pdf"
                ),
                dpi=200,
            )
    ...
    if "viz" in args.actions:
        ...
        testloader = torch.utils.data.DataLoader(
            test_data, batch_size=args.batch_size_test, shuffle=False, num_workers=1
        )
        ...
        if (
            attention_tracking is not None
            and hasattr(model, "kv_features")
            and model.kv_features is not None
        ):
            attention_tracking = attention_tracking.reshape(
            ...
            print(
                "Warning: Could not reshape attention_tracking. Visualizations may not include attention overlays."
            )
        ...
        for maze_i in range(inputs_viz.size(0)):
            ...
            current_input_np_original = (
                inputs_viz[maze_i].permute(1, 2, 0).detach().cpu().numpy()
            )
            ...
            current_input_np_display = (
                (current_input_np_original + 1) / 2
                if not args.legacy_scaling
                else current_input_np_original
            )
            ...
            try:
                solution_maze_img = draw_path(
                    current_input_np_display, current_target_route, gt=True
                )
                cv2.imwrite(
                    os.path.join(maze_output_dir, "solution_ground_truth.png"),
                    (solution_maze_img * 255).astype(np.uint8)[:, :, ::-1],
                )
            except Exception:  # Keep broad except for visualization robustness
                print(
                    f"Could not save ground truth solution for maze {maze_idx_display}"
                )
                pass
            ...
            for stepi in range(n_steps_viz):
                ...
                if (
                    ...
                    attn_resized = cv2.resize(
                        attn,
                        (
                            current_input_np_display.shape[1],
                            current_input_np_display.shape[0],
                        ),
                        interpolation=cv2.INTER_LINEAR,
                    )
                    if attn_resized.max() > attn_resized.min():
            ...
            if frames:
                imageio.mimsave(
                    os.path.join(maze_output_dir, "attention_overlay.gif"),
                    frames,
                    fps=15,
                    loop=0,
                )
