...
def make_maze_gif(
    ...
    with tqdm(
        ...
        for stepi in np.arange(0, predictions.shape[-1], 1):
            ...
            overlay_img = np.clip(
                guess_maze * (1 - mapped_attention[..., np.newaxis] * 0.6)
                + (
                ...
                * 1.1,
                0,
                1,
            )  # np.clip((np.copy(guess_maze)*(1-aggregated_attention[:,:,np.newaxis])*0.7 + (aggregated_attention[:,:,np.newaxis]*3 * np.reshape(np.array(colour)[:3], (1, 1, 3)))), 0, 1)
