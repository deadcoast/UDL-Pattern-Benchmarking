...
def make_parity_gif(
    ...
    mosaic = (
        ...
        + [
            ...
            for ti in range(n_neurons_to_visualise)
        ]
    )
    ...
    for stepi in tqdm(range(n_steps), desc="Processing steps", unit="step"):
        ...
        axes_gif["probs"].imshow(
            probs_grid, cmap="viridis", interpolation="nearest", vmin=0, vmax=1
        )
        ...
        axes_gif["target"].imshow(
            target_grid, cmap="viridis_r", interpolation="nearest", vmin=0, vmax=1
        )
...
def plot_attention_trajectory(
    ...
    trajectory = [
        np.unravel_index(np.argmax(attention[t]), (grid_size, grid_size))
    ...
    for i in range(len(trajectory) - 1):
        ...
        (line,) = plt.plot(
            [y1, y2], [x1, x2], color=color, linewidth=6, alpha=0.5, zorder=4
        )
    ...
    plt.plot(
        most_certain_point[1],
        most_certain_point[0],
        marker="x",
        markersize=18,
        markeredgewidth=5,
        color="white",
    ...
    plt.plot(
        ...
        markersize=15,
        markeredgewidth=3,
        color=cmap(norm_time[where_most_certain]),
        linestyle="",
        zorder=7,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        f"{save_dir}/{filename}_traj.png", dpi=300, bbox_inches="tight", pad_inches=0
    )
    plt.savefig(
        f"{save_dir}/{filename}_traj.pdf",
        format="pdf",
        bbox_inches="tight",
        pad_inches=0,
    )
...
def plot_input(input_images, save_dir, filename):
    ...
    plt.tight_layout()
    plt.savefig(
        f"{save_dir}/{filename}_input.png", dpi=300, bbox_inches="tight", pad_inches=0
    )
    plt.savefig(
        f"{save_dir}/{filename}_input.pdf",
        format="pdf",
        bbox_inches="tight",
        pad_inches=0,
    )
...
def plot_target(targets, save_dir, filename, args):
    ...
    plt.tight_layout()
    plt.savefig(
        f"{save_dir}/{filename}_target.png", dpi=300, bbox_inches="tight", pad_inches=0
    )
    plt.savefig(
        f"{save_dir}/{filename}_target.pdf",
        format="pdf",
        bbox_inches="tight",
        pad_inches=0,
    )
...
def plot_probabilities(predictions, certainties, save_dir, filename, args):
    grid_size = int(math.sqrt(args.parity_sequence_length))
    where_most_certain = get_where_most_certain(certainties)
    predictions_most_certain = (
        predictions[0, :, :, where_most_certain].detach().cpu().numpy()
    )
    ...
    plt.tight_layout()
    plt.savefig(
        f"{save_dir}/{filename}_probs.png", dpi=300, bbox_inches="tight", pad_inches=0
    )
    plt.savefig(
        f"{save_dir}/{filename}_probs.pdf",
        format="pdf",
        bbox_inches="tight",
        pad_inches=0,
    )
...
def plot_prediction(predictions, certainties, save_dir, filename, args):
    ...
    class_grid = np.argmax(predictions_most_certain, axis=1).reshape(
        grid_size, grid_size
    )
    ...
    plt.tight_layout()
    plt.savefig(
        f"{save_dir}/{filename}_prediction.png",
        dpi=300,
        bbox_inches="tight",
        pad_inches=0,
    )
    plt.savefig(
        f"{save_dir}/{filename}_prediction.pdf",
        format="pdf",
        bbox_inches="tight",
        pad_inches=0,
    )
...
def plot_accuracy_heatmap(
    ...
    im = ax.imshow(
        overall_accuracies_avg.T * 100,
        aspect="auto",
        cmap="viridis",
        origin="lower",
        extent=[0, args.iterations - 1, 0, args.parity_sequence_length - 1],
        vmin=50,
        vmax=100,
    )
    ...
    ax.errorbar(
        average_thinking_time,
        np.arange(args.parity_sequence_length),
        xerr=std_thinking_time,
        fmt="ko",
        markersize=2,
        capsize=2,
        elinewidth=1,
...
def plot_attention_heatmap(
    ...
    overall_attentions_avg = overall_attentions_avg.reshape(
        overall_attentions_avg.shape[0], -1
    )
    fig, ax = plt.subplots(figsize=(scale * 10, scale * 5))
    im = ax.imshow(
        overall_attentions_avg.T,
        aspect="auto",
        cmap="viridis",
        origin="lower",
        extent=[
            ...
            overall_attentions_avg.shape[0] - 1,
            0,
            overall_attentions_avg.shape[1] - 1,
        ],
        vmin=vmin,
        vmax=vmax,
    )
...
def create_accuracies_heatmap_gif(
    all_accuracies,
    ...
    for i, (accuracies, avg_thinking_time, std_thinking_time) in enumerate(
        zip(all_accuracies, all_average_thinking_times, all_std_thinking_times)
    ):
        save_path = os.path.join(heatmap_components_dir, f"frame_{i:04d}.png")
        plot_accuracy_heatmap(
            accuracies, avg_thinking_time, std_thinking_time, scale, save_path, args
        )
...
def create_stacked_gif(save_path, y_shift=200):
    ...
    for acc_frame, att_frame in zip(accuracy_frames, attention_frames):
        ...
        stacked_frame = Image.new(
            "RGB", (stacked_width, stacked_height), color=(255, 255, 255)
        )

        # Paste attention frame first, shifted up
        stacked_frame.paste(att_frame, (0, 0))  # Paste at top
        stacked_frame.paste(
            acc_frame, (0, att_height - y_shift)
        )  # Shift accuracy up by overlap
...
def plot_accuracy_training(all_accuracies, scale, run_model_spefic_save_dir, args):
    ...
    fig.tight_layout(pad=0.1)
    fig.savefig(
        f"{run_model_spefic_save_dir}/accuracy_vs_seq_element.png",
        dpi=300,
        bbox_inches="tight",
    )
    fig.savefig(
        f"{run_model_spefic_save_dir}/accuracy_vs_seq_element.pdf",
        format="pdf",
        bbox_inches="tight",
    )
    plt.close(fig)
...
def plot_loss_all_runs(
    training_data,
    evaluate_every,
    save_path="train_loss_comparison_parity.png",
    step=1,
    scale=1.0,
    x_max=None,
):
    ...
    for key in sorted(grouped.keys(), key=lambda k: (iters_map[k], model_map[k])):
        ...
        (line,) = ax.plot(
            x, mean, color=color, linestyle=linestyle, label=label_map[key]
        )
...
def plot_accuracy_all_runs(
    training_data,
    evaluate_every,
    save_path="test_accuracy_comparison_parity.png",
    step=1,
    scale=1.0,
    smooth=False,
    x_max=None,
    ...
    for i, (folder, data) in enumerate(training_data.items()):
        ...
        if model_type.lower() == "ctm":
            ...
            if memory_length is None:
                raise ValueError(
                    f"CTM model missing memory_length in checkpoint args from: {folder}"
                )
...
def plot_training_curve_all_runs(
    all_folders,
    save_dir,
    scale,
    device,
    ...
    assert all(
        interval == evaluation_intervals[0] for interval in evaluation_intervals
    ), "Evaluation intervals are not consistent across runs."

    evaluate_every = evaluation_intervals[0]

    if plot_individual_runs:
        plot_loss_individual_runs(
            training_data, evaluate_every, save_dir=save_dir, scale=scale, x_max=x_max
        )
        plot_accuracy_individual_runs(
            training_data,
            evaluate_every,
            save_dir=save_dir,
            scale=scale,
    ...
    plot_loss_all_runs(
        training_data,
        evaluate_every,
        save_path=f"{save_dir}/loss_comparison.png",
        scale=scale,
        x_max=x_max,
    )
    plot_accuracy_all_runs(
        training_data,
        evaluate_every,
...
def plot_lstm_last_and_certain_accuracy(
    all_folders,
    save_path="lstm_last_and_certain_accuracy.png",
    scale=1.0,
    step=1,
    x_max=None,
):
