...
def parse_args():
    ...
    parser.add_argument(
        "--log_dir",
        type=str,
        default="checkpoints/parity",
        help="Directory to save logs.",
    )
    parser.add_argument(
        "--batch_size_test", type=int, default=128, help="batch size for testing"
    )
    parser.add_argument(
        "--scale_training_curve",
        type=float,
        default=0.6,
        help="Scaling factor for plots.",
    )
    parser.add_argument(
        "--scale_heatmap",
        type=float,
        default=0.4,
        help="Scaling factor for heatmap plots.",
    )
    parser.add_argument(
        "--scale_training_index_accuracy",
        type=float,
        default=0.4,
        help="Scaling factor for training index accuracy plots.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=["ctm", "lstm"],
        default="ctm",
        help="Type of model to analyze (ctm or lstm).",
    )
...
def calculate_entropy_average_over_batch(normalized_entropy_per_elements):
    normalized_entropy_per_elements_avg_batch = normalized_entropy_per_elements.mean(
        axis=1
    )
...
def test_handcrafted_examples(model, args, run_model_spefic_save_dir, device):
    ...
    random_input = (
        torch.randint(
            0, 2, (args.parity_sequence_length,), dtype=torch.float32, device=device
        )
        * 2
        - 1
    )
...
def analyze_trained_model(run_model_spefic_save_dir, args, device):
    with torch.no_grad():
        ...
        test_data = ParityDataset(
            sequence_length=model_args.parity_sequence_length, length=10000
        )
        ...
        for inputs, targets in testloader:
            ...
            corrects_at_most_certain_time_batch = (
                get_corrects_per_element_at_most_certain_time(
...
def analyze_training(run_model_spefic_save_dir, args, device):
    ...
    for checkpoint_path in checkpoint_files:
        model, model_args = build_model_from_checkpoint_path(
            checkpoint_path, args.model_type, device=device
        )
        model_args.log_dir = run_model_spefic_save_dir
        test_data = ParityDataset(
            sequence_length=model_args.parity_sequence_length, length=1000
        )
        testloader = torch.utils.data.DataLoader(
            test_data,
            batch_size=args.batch_size_test,
        ...
        for inputs, targets in testloader:
            ...
            entropy_per_element = (
                compute_normalized_entropy(
                    predictions.permute(0, 3, 1, 2), reduction="none"
                )
                .detach()
                .cpu()
                .numpy()
        ...
        checkpoint_average_accuracies = np.mean(
            np.concatenate(corrects, axis=0), axis=0
        ).transpose(1, 0)
        all_accuracies.append(checkpoint_average_accuracies)

        stacked_corrects_at_most_certain_times = np.vstack(
            corrects_at_most_certain_times
        )
        checkpoint_average_accuracy_at_most_certain_time = np.mean(
            stacked_corrects_at_most_certain_times, axis=0
        )
        all_accuracies_at_most_certain_time.append(
            checkpoint_average_accuracy_at_most_certain_time
        )
        ...
        checkpoint_average_attentions = np.mean(
            np.concatenate(attentions, axis=1), axis=1
        )
        all_attentions.append(checkpoint_average_attentions)

    plot_accuracy_training(
        all_accuracies_at_most_certain_time,
        args.scale_training_index_accuracy,
        run_model_spefic_save_dir,
        args=model_args,
    )
    create_attentions_heatmap_gif(
        all_attentions, args.scale_heatmap, run_model_spefic_save_dir, model_args
    )
    create_accuracies_heatmap_gif(
...
if __name__ == "__main__":
    ...
    plot_training_curve_all_runs(
        all_runs_log_dirs, save_dir, args.scale_training_curve, device, x_max=200_000
    )
    plot_lstm_last_and_certain_accuracy(
        all_folders=all_runs_log_dirs,
        save_path=f"{save_dir}/lstm_final_vs_certain_accuracy.png",
    ...
    for folder in progress_bar:
        ...
        accuracy_mean, accuracy_std, num_iterations = analyze_trained_model(
    ...
    plot_accuracy_thinking_time(
        accuracy_csv_file_path, scale=args.scale_training_curve, output_dir=save_dir
    )
