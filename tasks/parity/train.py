...
def parse_args():
    ...
    parser.add_argument(
        "--d_input", type=int, default=512, help="Dimension of the input projection."
    )
    parser.add_argument(
        "--synapse_depth",
        type=int,
    ...
    parser.add_argument(
        "--n_synch_out", type=int, default=32, help="Number of neurons for output sync."
    )
    parser.add_argument(
        "--n_synch_action",
        type=int,
        default=32,
        help="Number of neurons for action sync.",
    )
    parser.add_argument(
        "--neuron_select_type",
        type=str,
    ...
    parser.add_argument(
        "--iterations", type=int, default=75, help="Number of internal ticks."
    )
    parser.add_argument(
        "--memory_length",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="custom-rotational-1d",
        help="Type of positional embedding.",
    )  # Choices removed for simplicity if not strictly needed by argparse functionality here
    parser.add_argument(
        "--backbone_type",
        type=str,
        default="parity_backbone",
    ...
    parser.add_argument(
        "--gamma", type=float, default=0.1, help="Scheduler gamma for multistep."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=-1,
        help="Quantile clipping value (-1 to disable).",
    )
    parser.add_argument(
    ...
    # Housekeeping
    parser.add_argument(
        "--log_dir", type=str, default="logs/parity", help="Directory for logging."
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="parity",
        help="Dataset name (used for assertion).",
    )
    parser.add_argument(
        "--save_every", type=int, default=1000, help="Save checkpoint frequency."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Num batches for metrics approx. (-1 for full).",
    )
    parser.add_argument(
        "--full_eval",
        action=argparse.BooleanOptionalAction,
...
if __name__ == "__main__":
    ...
    args.use_most_certain = args.model_type == "ctm" or (
        args.use_most_certain_with_lstm and args.model_type == "lstm"
    )
    ...
    if args.reload and (latest_checkpoint_path := get_latest_checkpoint(args.log_dir)):
        ...
        if not args.reload_model_only:
            ...
            train_accuracies_most_certain_per_input = (
                checkpoint["train_accuracies_most_certain_per_input"]
                if "train_accuracies_most_certain_per_input" in checkpoint
                else train_accuracies_most_certain_per_input
            )
            train_accuracies = checkpoint["train_accuracies"]
            test_losses = checkpoint["test_losses"]
            test_accuracies_most_certain = checkpoint["test_accuracies_most_certain"]
            test_accuracies_most_certain_per_input = (
                checkpoint["test_accuracies_most_certain_per_input"]
                if "test_accuracies_most_certain_per_input" in checkpoint
                else test_accuracies_most_certain_per_input
            )
    ...
    iterator = iter(
        trainloader
    )  # Not training in epochs, but rather iterations. Need to reset this from time to time
    with tqdm(
        ...
        for bi in range(start_iter, args.training_iterations):
            ...
            with torch.autocast(
                ...
                predictions = predictions.reshape(
                    predictions.size(0), -1, 2, predictions.size(-1)
                )
                loss, where_most_certain = parity_loss(
                    predictions,
                    certainties,
                    targets,
            ...
            if bi % args.track_every == 0:  # and bi != 0:
                ...
                with torch.inference_mode():
                    ...
                    plot_neural_dynamics(
                        post_activations, 100, args.log_dir, axis_snap=True
                    )
                    ...
                    with torch.inference_mode():
                        ...
                        with tqdm(
                            ...
                            for inferi, (inputs, targets) in enumerate(loader):
                                ...
                                these_predictions = reshape_predictions(
                                    these_predictions, prediction_reshaper
                                )
                                loss, where_most_certain = parity_loss(
                                    these_predictions,
                                    certainties,
                        ...
                        with tqdm(
                            ...
                            for inferi, (inputs, targets) in enumerate(loader):
                                ...
                                these_predictions = these_predictions.reshape(
                                    these_predictions.size(0),
                                    -1,
                                    2,
                        ...
                        if args.dataset != "sort":
                            for ti, (train_acc, test_acc) in enumerate(
                                zip(
                                    np.array(train_accuracies).T,
                                    np.array(test_accuracies).T,
                                )
                            ):
                                axacc_train.plot(
                                    iters,
