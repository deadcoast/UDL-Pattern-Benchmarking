...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="ctm",
        choices=["ctm", "lstm"],
        help="Sequence length for parity task.",
    )
    parser.add_argument(
        "--d_model", type=int, default=128, help="Dimension of the model."
    )
    parser.add_argument(
        "--d_input", type=int, default=64, help="Dimension of the input projection."
    )
    parser.add_argument(
        "--synapse_depth",
        type=int,
        default=1,
        help="Depth of U-NET model for synapse. 1=linear.",
    )
    parser.add_argument(
        "--n_synch_out", type=int, default=16, help="Number of neurons for output sync."
    )
    parser.add_argument(
        "--neuron_select_type",
        type=str,
    ...
    parser.add_argument(
        "--iterations", type=int, default=1, help="Number of internal ticks."
    )
    parser.add_argument(
        "--memory_length",
        type=int,
        default=5,
        help="Length of pre-activation history for NLMs.",
    )
    parser.add_argument(
        "--deep_memory",
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Flag to carry over state trace between environment steps.",
    )

    # Environment Configuration
    parser.add_argument(
        "--env_id", type=str, default="Acrobot-v1", help="Environment ID."
    )
    parser.add_argument(
        "--mask_velocity",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Mask the velocity components of the observation.",
    )
    parser.add_argument(
        "--max_environment_steps",
        type=int,
        default=500,
        help="The maximum number of environment steps.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="The number of environment steps to run in each environment per policy rollout.",
    )
    parser.add_argument(
        "--total_timesteps",
        type=int,
        default=1_000_000,
        help="The combined total of all environment steps (across all batches).",
    )
    parser.add_argument(
        "--num_envs",
        type=int,
    ...
    parser.add_argument(
        "--discount_gamma", type=float, default=0.99, help="The discount factor gamma."
    )
    parser.add_argument(
        "--gae_lambda",
        type=float,
        default=0.95,
        help="The lambda for the Generalized Advantage Estimation (GAE).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Use clipped loss for the value function (as per the PPO paper).",
    )
    parser.add_argument(
        "--ent_coef", type=float, default=0.1, help="Entropy coefficient."
    )
    parser.add_argument(
        "--vf_coef", type=float, default=0.25, help="Value function coefficient."
    )
    parser.add_argument(
        "--max_grad_norm",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of environments to evaluate with during training.",
    )

    # Housekeeping
    parser.add_argument(
        "--log_dir", type=str, default="logs/rl/acrobot", help="Directory for logging."
    )
    parser.add_argument(
        "--tb_log_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="default_run",
        help="Name of the run for logging and tracking.",
    )
    parser.add_argument(
        "--save_every", type=int, default=100, help="Save checkpoint frequency."
    )
    ...
    parser.add_argument(
        "--track_every", type=int, default=1000, help="Track metrics frequency."
    )
    parser.add_argument(
        "--device", type=int, nargs="+", default=[-1], help="GPU(s) or -1 for CPU."
    )
...
class Agent(nn.Module):
    def __init__(self, size_action_space, args, device):
        ...
        if args.model_type == "ctm":
            ...
            actor_input_dim = critic_input_dim = (
                self.recurrent_model.synch_representation_size_out
            )
        else:
    ...
    def get_initial_ctm_state(self, num_envs):
        initial_state_trace = torch.repeat_interleave(
    ...
    def get_initial_lstm_state(self, num_envs):
        initial_hidden_state = torch.repeat_interleave(
            self.recurrent_model.start_hidden_state.unsqueeze(0), num_envs, 0
        )
        initial_cell_state = torch.repeat_interleave(
            self.recurrent_model.start_cell_state.unsqueeze(0), num_envs, 0
        )
    ...
    def _get_lstm_hidden_states(self, lstm_state, done, num_envs):
        ...
        return (masked_previous_hidden_state + masked_initial_hidden_state), (
            masked_previous_cell_state_state + masked_initial_cell_state
        )

    def _get_ctm_hidden_states(self, ctm_state, done, num_envs):
        initial_state_trace, initial_activated_state_trace = self.get_initial_ctm_state(
            num_envs
        )
        if self.continious_state_trace:
            ...
            return (masked_previous_state_trace + masked_initial_state_trace), (
                masked_previous_activated_state_trace
                + masked_initial_activated_state_trace
            )
        else:
            return (initial_state_trace, initial_activated_state_trace)
    ...
    def get_states(self, x, ctm_state, done, track=False):
        ...
        for x, d in zip(xs, done):
            if not track:
                synchronisation, ctm_state = self.recurrent_model(
                    x, self._get_hidden_states(ctm_state, d, num_envs)
                )
                tracking_data = None
                new_hidden += [synchronisation]
            else:
                synchronisation, ctm_state, pre_activations, post_activations = (
                    self.recurrent_model(
                        x, self._get_hidden_states(ctm_state, d, num_envs), track=True
                    )
                )
                tracking_data = {
    ...
    def get_action_and_value(self, x, ctm_state, done, action=None, track=False):
        hidden, ctm_state, tracking_data = self.get_states(
            x, ctm_state, done, track=track
        )
...
def load_model(agent, optimizer, checkpoint_path, device):
    ...
    print(
        f"Loaded checkpoint from {checkpoint_path} at iteration {training_iteration} and step {global_step}"
    )
    return (
        global_step,
        training_iteration,
    ...
    with torch.no_grad():
        for idx in range(args.num_validation_envs):
            if args.env_id in ("CartPole-v1", "Acrobot-v1"):
                eval_env = make_env_classic_control(
                    args.env_id,
                    args.max_environment_steps,
                    mask_velocity=args.mask_velocity,
                    render_mode="rgb_array",
                )()
            ...
            for _ in range(args.max_environment_steps):
                ...
                tracking_data["actions"] = np.tile(
                    action.detach().cpu().numpy(), (args.iterations)
                )  # Shape T
                tracking_data["values"] = np.tile(
                    value.squeeze(-1).detach().cpu().numpy(), (args.iterations)
                )  # Shape T
                tracking_data["action_logits"] = np.tile(
                    action_logits.detach().cpu().numpy(), (args.iterations, 1)
                )  # Shape T, A
                tracking_data["action_probs"] = np.tile(
                    action_probs.detach().cpu().numpy(), (args.iterations, 1)
                )  # Shape T, A
                tracking_data["rewards"] = np.tile(
                    np.array(reward), (args.iterations)
                )  # Shape T
                tracking_data["inputs"] = np.tile(
                    np.array(eval_env.render()), (args.iterations, 1, 1, 1)
                )  # Shape T, H, W, C
            ...
            n_to_plot = (
                80
                if combined_tracking_data["post_activations"].shape[-1] < 100
                else 100
            )
...
if __name__ == "__main__":
        ...
        envs = gym.vector.SyncVectorEnv(
            [
                make_env_classic_control(
                    args.env_id, args.max_environment_steps, args.mask_velocity
                )
    ...
    progress_bar = tqdm(
        range(training_iteration, args.num_training_iterations + 1),
    ...
    for training_iteration in progress_bar:
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value, next_state, _, _, _ = (
                    agent.get_action_and_value(next_obs, next_state, next_done)
                )
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(args.num_steps)):
                ...
                delta = (
                    rewards[t]
                    + args.discount_gamma * nextvalues * nextnonterminal
                    - values[t]
                )
            ...
            for start in range(0, args.num_envs, envsperbatch):
                ...
                if args.model_type == "ctm":
                    selected_hidden_state = (
                        initial_state[0][mbenvinds, :, :],
                        initial_state[1][mbenvinds, :, :],
                    )
                ...
                _, newlogprob, entropy, newvalue, _, _, _, _ = (
                    agent.get_action_and_value(
                        b_obs[mb_inds],
                        selected_hidden_state,
                        b_dones[mb_inds],
        ...
        if (
            training_iteration % args.save_every == 0
            or training_iteration == 1
            or global_step == args.total_timesteps - 1
        ):
            save_model(
                agent,
                optimizer,
