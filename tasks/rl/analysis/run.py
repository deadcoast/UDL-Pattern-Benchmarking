...
def parse_args():
    ...
    parser.add_argument(
        "--log_dir", type=str, default="checkpoints/rl", help="Directory to save logs."
    )
    parser.add_argument(
        "--scale", type=float, default=0.5, help="Scaling factor for plot size"
    )
    parser.add_argument(
        "--num_eval_envs",
        type=int,
...
def get_human_readable_name(checkpoint_path):
    ...
    if match:
        ...
        raise ValueError(
            f"Could not extract human readable name from {checkpoint_path}"
        )


def compute_mean_std_over_runs(
    ...
    interpolated = [
        interp1d(steps, values, bounds_error=False, fill_value="extrapolate")(
            common_steps
        )
...
def plot(
    ...
    for i, (key, data) in enumerate(final_order):
        ...
        common_steps, mean, std = compute_mean_std_over_runs(
            steps_list, values_list, num_interpolation_points, window_length
        )
        color = colors[i]
    ...
    for env_id in ("CartPole-v1", "Acrobot-v1", "MiniGrid-FourRooms-v0"):
        ...
        if grouped_data:
            plot(
                ...
                title="Avg Episode Rewards ± Std",
                save_path=f"{save_dir}/{env_id}/episode_rewards_avg.png",
            )
            plot(
                grouped_data,
                scale=args.scale,
                ...
                title="Avg Episode Lengths ± Std",
                save_path=f"{save_dir}/{env_id}/episode_lengths_avg.png",
            )
        else:
...
def prepare_env(env_id, max_environment_steps, mask_velocity, render_mode):
    if env_id in ("CartPole-v1", "Acrobot-v1"):
        return make_env_classic_control(
            env_id,
            max_environment_steps,
            mask_velocity=mask_velocity,
        ...
        for arch in ARCHS_TO_TEST:
            for iters in ITERS_TO_TEST:
                ...
                for run in RUNS_TO_TEST:
                    ...
                    _, _, _, _, _, _ = load_model(
                        agent, optimizer, checkpoints[0], device
                    )
                    ...
                    for idx in range(args.num_eval_envs):
                        ...
                        for environment_step in range(model_args.max_environment_steps):
                            ...
                            eval_next_obs, reward, termination, truncation, _ = (
                                eval_env.step(action.cpu().numpy()[0])
                            )
                            ...
                            tracking_data["values"] = np.tile(
                                value.squeeze(-1).detach().cpu().numpy(),
                                (model_args.iterations),
                            )  # Shape T
                            tracking_data["action_logits"] = np.tile(
                                action_logits.detach().cpu().numpy(),
                                (model_args.iterations, 1),
                            )  # Shape T, A
                        ...
                        n_to_plot = (
                            80
                            if combined_tracking_data["post_activations"].shape[-1]
                            < 100
                            else 100
                        )
                        if idx == 0:
