...
def make_rl_gif(
    ...
    for stepi in range(n_steps):
        ...
        colors = [
            "black" if i == actions[stepi] else ("b" if e >= 0 else "r")
            for i, e in enumerate(these_action_logits[stepi])
        ]
        sort_idxs = np.arange(len(these_action_logits[stepi]))
        bars = axes_gif["action_logits"].bar(
            np.arange(len(these_action_logits[stepi][sort_idxs])),
            these_action_logits[stepi][sort_idxs],
        ...
        for bar, label in zip(bars, class_labels):
            ...
            axes_gif["action_logits"].annotate(
                label,
                xy=(x, 0),
                xytext=(1, 0),
        ...
        axes_gif["action_logits"].set_ylim(
            [np.min(these_action_logits), np.max(these_action_logits)]
        )
        ...
        bars = axes_gif["action_log_probs"].bar(
            np.arange(len(these_action_log_probs[stepi][sort_idxs])),
            these_action_log_probs[stepi][sort_idxs],
            color=np.array(colors)[sort_idxs],
            width=0.9,
            alpha=0.5,
        )
        ...
        for bar, label in zip(bars, class_labels):
            x = bar.get_x() + bar.get_width() / 2
            axes_gif["action_log_probs"].annotate(
                label,
                xy=(x, 0),
                xytext=(1, 0),
                textcoords="offset points",
