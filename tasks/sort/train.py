...
def parse_args():
    ...
    parser.add_argument(
        "--d_input", type=int, default=128, help="Dimension of the input."
    )
    parser.add_argument(
        "--synapse_depth",
        type=int,
...
if __name__ == "__main__":
    ...
    if args.reload:
        if os.path.isfile(f"{args.log_dir}/checkpoint.pt"):
            ...
            checkpoint = torch.load(
                f"{args.log_dir}/checkpoint.pt", map_location=device, weights_only=False
            )
    ...
    with tqdm(
        ...
        for bi in range(start_iter, args.training_iterations):
            ...
            accuracy = compute_ctc_accuracy(
                predictions, targets, predictions.shape[1] - 1
            )
                    ...
                    with torch.inference_mode():
                        ...
                        with tqdm(
                            ...
                            for inferi, (inputs, targets) in enumerate(loader):
                                ...
                                decoded = [
                                    d[: targets.shape[1]]
                                    for d in decode_predictions(
                                        these_predictions, predictions.shape[1] - 1
                                    )
                                ]
                                decoded = torch.stack(
                                    [
