...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of inputs per min mnist view",
    )
    parser.add_argument(
        "--q_num_images_delta",
        type=int,
        default=2,
        help="Range of numbers for QMNIST dataset",
    )
    parser.add_argument(
        "--q_num_repeats_per_input",
        type=int,
        default=10,
        help="Number of MNIST repeats to show model",
    )
    parser.add_argument(
        "--q_num_operations",
        type=int,
        default=3,
        help="The number of operations to apply.",
    )
    parser.add_argument(
        "--q_num_operations_delta",
        type=int,
        default=2,
        help="The range of operations to apply.",
    )
    parser.add_argument(
        "--q_num_answer_steps",
        type=int,
        default=10,
        help="The number of steps to answer a question, after observing digits and operator embeddings.",
    )
    ...
    parser.add_argument(
        "--d_input", type=int, default=64, help="Dimension of the input."
    )
    parser.add_argument(
        "--synapse_depth",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Number of neurons to use for output synch.",
    )
    parser.add_argument(
        "--n_synch_action",
        type=int,
    ...
    parser.add_argument(
        "--iterations", type=int, default=50, help="Number of internal ticks."
    )
    parser.add_argument(
        "--memory_length",
        type=int,
        default=30,
        help="Length of the pre-activation history for NLMS.",
    )
    parser.add_argument(
        "--deep_memory",
    ...
    # Housekeeping
    parser.add_argument(
        "--log_dir", type=str, default="logs/qamnist", help="Directory for logging."
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    trainloader = torch.utils.data.DataLoader(
        train_data, num_workers=0, batch_sampler=train_sampler
    )

    test_sampler = QAMNISTSampler(test_data, batch_size=args.batch_size_test)
    testloader = torch.utils.data.DataLoader(
        test_data, num_workers=0, batch_sampler=test_sampler
    )
    ...
    with tqdm(
        ...
        for bi in range(start_iter, args.training_iterations):
            ...
            accuracy = (
                (
                    predictions_answer_steps.argmax(1)[
                        torch.arange(
                            predictions_answer_steps.size(0), device=predictions.device
                        ),
                        where_most_certain,
                    ]
                    == targets
            ...
            pbar.set_description(
                f"Dataset=QAMNIST. Loss={loss.item():0.3f}. Accuracy={accuracy:0.3f}. LR={current_lr:0.6f}. Where_certain={where_most_certain.float().mean().item():0.2f}+-{where_most_certain.float().std().item():0.2f} ({where_most_certain.min().item():d}<->{where_most_certain.max().item():d})"
            )
            ...
            if bi % args.track_every == 0:
                ...
                with torch.inference_mode():
                    ...
                    embedding_padded = np.pad(
                        embedding_tracking, pad_width, mode="constant"
                    )
                    ...
                    embedding_tensor = torch.from_numpy(embedding_input).to(
                        gif_inputs.device
                    )
                    gif_inputs[
                    ...
                    iters.append(bi)
                    pbar.set_description(
                        "Tracking: Computing loss and accuracy for curves"
                    )
                    with torch.inference_mode():
                        ...
                        with tqdm(
                            ...
                            for inferi, (
                                ...
                                loss, where_most_certain = qamnist_loss(
                                    these_predictions_answer_steps,
                                    certainties_answer_steps,
                                    targets,
                                    use_most_certain=args.use_most_certain,
                                )
                                ...
                                if (
                                    ...
                                    and inferi % args.n_test_batches == 0
                                    and inferi != 0
                                ):
                                    break
