...
def parse_args():
    ...
    parser.add_argument(
        "--generalization_heatmap_scale",
        type=float,
        default=0.5,
        help="Scaling for the generalization heatmap.",
    )
...
def plot_accuracy_operations(
    ...
    ax.set_title(
        "Accuracy vs Number of Operations for Different Number of Digits Shown"
    )
...
def plot_loss_all_runs(
    training_data,
    evaluate_every,
    save_path="train_loss_comparison_qamnist.png",
    window_size=1000,
):
    ...
    legend_handles = []
    all_losses = [
        loss for data in training_data.values() for loss in data["train_losses"]
    ]
    ...
    for (folder, data), color in zip(training_data.items(), colors):
        ...
        smooth_losses = np.convolve(
            data["train_losses"], np.ones(window_size) / window_size, mode="same"
        )
...
def get_accuracy(testloader, model, device, args):
    ...
    with torch.inference_mode():
        ...
        if args.model_type == "lstm":
            ...
            raise ValueError(
                "Only CTM and LSTM models work with this accuracy calculation."
            )
    ...
    for folder in progress_bar:
        ...
        for i, num_digits in enumerate(num_digits_to_test):
            for j, num_operations in enumerate(num_operations_to_test):
                ...
                acc_values = [
                    get_accuracy(testloader, model, device, model_args)
        ...
        df_results.to_csv(
            f"{run_model_spefic_save_dir}/accuracy_results_{model_args.model_type}_{model_args.q_num_repeats_per_input}.csv",
            index=False,
        )
        ...
        plot_accuracy_grid(
            ...
            num_digits_to_test,
            num_operations_to_test,
            args.generalization_heatmap_scale,
            filename=f"{run_model_spefic_save_dir}/accuracy_grid_{model_args.model_type}_{model_args.q_num_repeats_per_input}.png",
        )
        plot_accuracy_operations(
            accuracy_matrix,
            std_matrix,
            num_operations_to_test,
            num_digits_to_test,
            filename=f"{run_model_spefic_save_dir}/accuracy_scatter_{model_args.model_type}_{model_args.q_num_repeats_per_input}.png",
        )
