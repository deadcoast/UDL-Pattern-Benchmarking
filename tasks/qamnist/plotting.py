...
def make_qamnist_gif(
    ...
    for stepi in range(n_steps):
        ...
        colors = [
            ("g" if i == this_target else ("b" if e >= 0 else "r"))
            for i, e in enumerate(these_predictions[:, stepi])
        ]
        sort_idxs = np.arange(len(these_predictions[:, stepi]))
        bars = axes_gif["logits"].bar(
            np.arange(len(these_predictions[:, stepi])),
            these_predictions[:, stepi][sort_idxs],
