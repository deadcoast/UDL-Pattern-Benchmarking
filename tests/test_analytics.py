...
def sample_reports():
    ...
    for i in range(30):
        for j, udl_file in enumerate(udl_files):
            ...
            metric_scores = {
                'ConsistencyMetric': max(0.0, min(1.0, overall_score + np.random.normal(0, 0.1))),
                'CompletenessMetric': max(0.0, min(1.0, overall_score + np.random.normal(0, 0.1))),
                'ExpressivenessMetric': max(0.0, min(1.0, overall_score + np.random.normal(0, 0.1))),
                'StructuralCoherenceMetric': max(0.0, min(1.0, overall_score + np.random.normal(0, 0.1)))
            }
            ...
            report = QualityReport(
                ...
                computation_trace=[],
                error_bounds={k: (v - 0.05, v + 0.05) for k, v in metric_scores.items()},
...
class TestPortfolioAnalyzer:
    ...
    def test_project_mapping(self, sample_reports):
        ...
        assert comparison.project_profiles['ProjectA'].total_reports > comparison.project_profiles['ProjectB'].total_reports
...
class TestImprovementAdvisor:
    ...
    def test_generate_improvement_plan(self, sample_reports):
        ...
        for suggestion in plan.suggestions:
            assert suggestion.category in ['consistency', 'completeness', 'expressiveness', 'structure', 'errors', 'warnings', 'trends', 'confidence']
...
class TestBusinessIntelligenceExporter:
    ...
    def test_aggregation_levels(self, sample_reports):
        ...
        dataset_executive = exporter.export_quality_data(sample_reports, config_executive)
