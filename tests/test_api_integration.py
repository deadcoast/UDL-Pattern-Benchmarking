...
class TestEndpointValidation:
    ...
    def test_batch_endpoint_validation(self):
        ...
        response = self.client.post("/rate/batch", json={"udls": [{"invalid": "structure"}]})
        assert response.status_code == 422
...
class TestBatchProcessing:
    ...
    def test_batch_parallel_processing(self, mock_pipeline):
        ...
        response = self.client.post("/rate/batch", json={
            "udls": udls,
            "parallel": True
        })
    ...
    def test_batch_partial_failure(self):
        ...
        with patch('main.rating_pipeline') as mock_pipeline:
            ...
            udls = [
                {"content": "grammar Test1 { rule = 'hello1' }", "filename": "test1.udl"},
                {"content": "grammar Test2 { rule = 'hello2' }", "filename": "test2.udl"},
                {"content": "grammar Test3 { rule = 'hello3' }", "filename": "test3.udl"}
            ]
...
class TestMetricsEndpoint:
    ...
    def test_metrics_with_real_pipeline(self, mock_pipeline):
        ...
        mock_metric1.get_formula.return_value = "1 - (|Contradictions| + |Cycles|) / (|Rules| + 1)"
    ...
    def test_metrics_with_faulty_metric(self, mock_pipeline):
        ...
        faulty_metric = next((m for m in data["metrics"] if m["name"] == "FaultyMetric"), None)
        if faulty_metric:
