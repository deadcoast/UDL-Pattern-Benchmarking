...
class TestDocstringSignatureAccuracy:
    ...
    **Validates: Requirements 8.1, 8.2**
    ...
    @pytest.mark.skipif(
        len(DOCUMENTED_FUNCTIONS) == 0,
        reason="No documented functions found"
    )
    @given(st.sampled_from(DOCUMENTED_FUNCTIONS) if DOCUMENTED_FUNCTIONS else st.nothing())
    @settings(max_examples=min(100, len(DOCUMENTED_FUNCTIONS)) if DOCUMENTED_FUNCTIONS else 1, deadline=None)
    ...
    **Validates: Requirements 8.5**
    ...
    def test_metric_docstrings_contain_formulas(self):
        ...
        for metric_class in metrics:
            ...
            has_math = any(keyword in docstring.lower() for keyword in [
                'mathematical', 'formula', 'definition', '=', 'where:'
            ])
            assert has_math, f"{metric_class.__name__} docstring lacks mathematical definition"
...
class TestDocstringValidatorIntegration:
    ...
    def test_signature_mismatches_have_details(self):
        ...
        for mismatch in report.signature_mismatches:
            ...
            assert mismatch.mismatch_type in ('missing_param', 'extra_param', 'type_mismatch')
