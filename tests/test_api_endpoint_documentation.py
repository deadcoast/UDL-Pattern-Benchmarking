...
def extract_documented_endpoints() -> List[Dict[str, str]]:
    ...
    endpoint_pattern = r'[`\-\*]\s*`?(GET|POST|PUT|DELETE|PATCH)\s+(/[^\s`]+)`?\s*[-–]\s*(.+?)(?:\n|$)'
    ...
    **Validates: Requirements 12.3**
    ...
    def test_all_endpoints_are_documented(self):
        ...
        **Validates: Requirements 12.3**
        ...
        # Report findings
        if undocumented:
            undocumented_list = [f"{method} {path}" for method, path in sorted(undocumented)]
            pytest.fail(
    ...
    def test_endpoint_documentation_completeness(self):
        ...
        **Validates: Requirements 12.3**
        ...
        incomplete = [
            e for e in documented_endpoints 
    ...
    def test_endpoint_count_matches(self):
        ...
        **Validates: Requirements 12.3**
        ...
        coverage = len(actual_keys & documented_keys) / len(actual_keys) if actual_keys else 1.0
...
class TestAPIDocumentationConsistency:
    ...
    **Validates: Requirements 12.3**
    ...
    def test_readme_has_api_section(self):
        ...
        assert 'API Endpoints' in content or 'API endpoints' in content or '## API' in content, \
    ...
    def test_client_library_matches_api(self):
        ...
        **Validates: Requirements 12.3**
        ...
        # Read Python client
        client_path = Path(__file__).parent.parent / "deployment" / "client" / "python_client.py"
...
def test_http_methods_documented_consistently(method: str):
    ...
    *For any* HTTP method used in the API, if endpoints use that method,
    ...
    assert not missing, (
        f"Endpoints using {method} not documented: {missing}"
    )
