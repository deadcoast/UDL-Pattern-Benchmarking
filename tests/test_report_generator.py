...
class TestReportGenerator:
    ...
    def sample_computation_steps(self) -> List[ComputationStep]:
        ...
        return [
            ComputationStep(
                ...
                formula="",
                inputs={"udl_file": "test.udl", "metrics": ["consistency", "completeness"]},
            ...
            ComputationStep(
                ...
                inputs={"weights": {"consistency": 0.5, "completeness": 0.5}, "metric_values": {"consistency": 0.85, "completeness": 0.75}},
                output=0.80,
    ...
    @pytest.fixture
    def sample_quality_report_with_errors(self, sample_computation_steps) -> QualityReport:
        ...
        return QualityReport(
            ...
            errors=["Failed to compute expressiveness metric: Division by zero", "Failed to compute structural coherence: Graph is empty"],
            warnings=["Input file has unusual structure", "Some tokens could not be classified"]
        )
    ...
    def test_json_report_single_report(self, report_generator: ReportGenerator, sample_quality_report: QualityReport):
    ...
    def test_json_report_multiple_reports(self, report_generator: ReportGenerator, sample_quality_report: QualityReport, sample_quality_report_with_errors: QualityReport):
        ...
        assert summary["overall_score"]["mean"] == pytest.approx((0.80 + 0.45) / 2, rel=1e-3)
        assert summary["errors"]["total_errors"] == 2
        assert summary["errors"]["total_warnings"] == 3
    
    def test_json_report_file_output(self, report_generator: ReportGenerator, sample_quality_report: QualityReport):
        """Test JSON report generation with file output."""
        with tempfile.TemporaryDirectory() as temp_dir:
            output_path = Path(temp_dir) / "test_report.json"
            
            json_str = report_generator.generate_json_report(sample_quality_report, output_path=output_path)
    ...
    def test_csv_report_single_report(self, report_generator: ReportGenerator, sample_quality_report: QualityReport):
        ...
        assert float(row["error_bound_consistency_lower"]) == pytest.approx(0.80, rel=1e-3)
        assert float(row["error_bound_consistency_upper"]) == pytest.approx(0.90, rel=1e-3)
    
    def test_csv_report_multiple_reports(self, report_generator: ReportGenerator, sample_quality_report: QualityReport, sample_quality_report_with_errors: QualityReport):
    ...
    def test_csv_report_file_output(self, report_generator: ReportGenerator, sample_quality_report: QualityReport):
        """Test CSV report generation with file output."""
        with tempfile.TemporaryDirectory() as temp_dir:
            output_path = Path(temp_dir) / "test_report.csv"
            
            csv_str = report_generator.generate_csv_report(sample_quality_report, output_path=output_path)
    ...
    def test_html_report_single_report(self, report_generator: ReportGenerator, sample_quality_report: QualityReport):
    ...
    def test_html_report_multiple_reports(self, report_generator: ReportGenerator, sample_quality_report: QualityReport, sample_quality_report_with_errors: QualityReport):
    ...
    def test_html_report_file_output(self, report_generator: ReportGenerator, sample_quality_report: QualityReport):
        """Test HTML report generation with file output."""
        with tempfile.TemporaryDirectory() as temp_dir:
            output_path = Path(temp_dir) / "test_report.html"
            
            html_str = report_generator.generate_html_report(sample_quality_report, output_path=output_path)
    ...
    def test_html_report_custom_title(self, report_generator: ReportGenerator, sample_quality_report: QualityReport):
        """Test HTML report generation with custom title."""
        custom_title = "Custom UDL Analysis Report"
        html_str = report_generator.generate_html_report(sample_quality_report, title=custom_title)
        
        assert custom_title in html_str
        assert f"<title>{custom_title}</title>" in html_str
    
    def test_report_generator_without_visualizations(self, sample_quality_report: QualityReport):
    ...
    def test_large_numbers_precision(self, report_generator: ReportGenerator):
        ...
        json_data = json.loads(json_str)
        assert json_data["reports"][0]["overall_score"] == pytest.approx(0.123456789, rel=1e-9)
        assert json_data["reports"][0]["metric_scores"]["precise_metric"] == pytest.approx(0.111111111, rel=1e-9)
        
        # CSV should maintain precision
        csv_str = report_generator.generate_csv_report(precision_report)
        csv_reader = csv.DictReader(StringIO(csv_str))
        csv_rows = list(csv_reader)
        assert float(csv_rows[0]["overall_score"]) == pytest.approx(0.123456789, rel=1e-9)
