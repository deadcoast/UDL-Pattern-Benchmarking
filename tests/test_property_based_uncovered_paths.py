...
def udl_content_strategy(draw):
    ...
    for i in range(num_rules):
        lhs = draw(st.text(alphabet="ABCDEFGHIJKLMNOPQRSTUVWXYZ", min_size=1, max_size=5))
        rhs_parts = draw(st.lists(
...
class TestMathematicalComputationsEdgeCases:
    ...
    def test_confidence_calculation_edge_values(self, probs):
        ...
        if len(probs) == 1:
            ...
            assert confidence == 1.0, f"Single probability should give confidence 1.0, got {confidence}"
    ...
    def test_aggregation_mathematical_properties(self, weights):
        ...
        for metric_name, weight in weights.items():
            if weight > 0:
                ...
                increased_values[metric_name] = min(1.0, increased_values[metric_name] + 0.1)
    ...
    def test_aggregation_extreme_values(self, metric_values, noise):
        ...
        assert 0.0 <= result <= 1.0, f"Result {result} not bounded with values {metric_values}"
        
        # Test with clamped values (should be same as original if already in bounds)
        clamped_dict = {name: max(0.0, min(1.0, val)) for name, val in metric_dict.items()}
    ...
    def test_udl_representation_invariants(self, udl_content):
        ...
        try:
            ...
            if not udl_content.strip():
                assert len(tokens1) >= 0, "Empty content should produce non-negative token count"
                
        except Exception as e:
            # If parsing fails, it should fail consistently
            try:
                udl_repr2 = UDLRepresentation(udl_content, "test2.udl")
                pytest.fail(f"Parsing should fail consistently, but succeeded on second try: {e}")
            except:
    ...
    def test_quality_report_aggregation_invariants(self, metric_dicts):
        ...
        for i, metrics in enumerate(metric_dicts):
            ...
            report = QualityReport(
                ...
                computation_trace=[],
                error_bounds={name: (val * 0.9, val * 1.1) for name, val in metrics.items()},
                timestamp=None,
    ...
    def test_pipeline_state_consistency(self, file_score_pairs):
        ...
        try:
            for content, expected_score in file_score_pairs:
                temp_file = tempfile.NamedTemporaryFile(mode='w', suffix='.udl', delete=False)
            ...
            for i, (r1, r2) in enumerate(zip(results1, results2)):
                if r1 is not None and r2 is not None:
                    ...
                    pytest.fail(f"Inconsistent failure pattern for file {i}: {r1} vs {r2}")
...
class MetricComputationStateMachine(RuleBasedStateMachine):
    ...
    @rule(
        ...
        content=st.text(alphabet="abcdefghijklmnopqrstuvwxyz ::=|()[]{}+*", min_size=5, max_size=100)
    )
    ...
    @rule(
        metric_name=st.sampled_from(['consistency', 'completeness', 'expressiveness', 'structural_coherence'])
    )
...
class TestShrinkingAndMinimalExamples:
    ...
    def test_udl_parsing_robustness(self, content):
        ...
        try:
            ...
            assert hasattr(graph, 'number_of_nodes'), "Graph should have node count method"
            ...
            assert isinstance(e, Exception), f"Should raise proper exception, got {type(e)}"
    
    @given(
        arrays(
            dtype=np.float64,
            shape=st.integers(min_value=2, max_value=20),
            elements=st.floats(min_value=0.001, max_value=100.0, allow_nan=False, allow_infinity=False)
        )
    )
    @settings(max_examples=100, deadline=3000)
    def test_confidence_calculation_numerical_stability(self, raw_probs):
        ...
        try:
            ...
            assert np.isfinite(confidence), f"Confidence should be finite, got {confidence}"
            assert 0.0 <= confidence <= 1.0, f"Confidence {confidence} not in [0,1]"
            ...
            assert np.isfinite(tiny_confidence), f"Tiny confidence should be finite, got {tiny_confidence}"
            assert 0.0 <= tiny_confidence <= 1.0, f"Tiny confidence {tiny_confidence} not in [0,1]"
