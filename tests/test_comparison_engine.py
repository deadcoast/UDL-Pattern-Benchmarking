...
from udl_rating_framework.evaluation.comparison import (
    ComparisonEngine, ComparisonResult, RankingResult, ComparisonSummary
)
...
def quality_report_strategy(draw, min_score=0.0, max_score=1.0, num_metrics=4):
    ...
    overall_score = draw(st.floats(min_value=min_score, max_value=max_score, allow_nan=False, allow_infinity=False))
    confidence = draw(st.floats(min_value=0.0, max_value=1.0, allow_nan=False, allow_infinity=False))
    
    # Generate metric scores
    metric_names = [f"metric_{i}" for i in range(num_metrics)]
    metric_scores = {
        name: draw(st.floats(min_value=0.0, max_value=1.0, allow_nan=False, allow_infinity=False))
    ...
    udl_file = draw(st.text(min_size=1, max_size=20, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'))))
...
class TestComparisonEngineProperties:
    ...
    def test_property_29_consistent_rating_procedures(self, reports: List[QualityReport]):
        """
    ...
    @given(multiple_reports_strategy(min_reports=2, max_reports=5))
    @settings(max_examples=50, deadline=5000)
    def test_property_30_pairwise_difference_computation(self, reports: List[QualityReport]):
        """
        **Feature: udl-rating-framework, Property 30: Pairwise Difference Computation**
        ...
        for result in summary.pairwise_results:
            ...
            assert -1.0 <= result.difference <= 1.0, f"Difference out of range: {result.difference}"
    
    @given(multiple_reports_strategy(min_reports=2, max_reports=5))
    @settings(max_examples=50, deadline=5000)
    def test_property_31_statistical_significance_testing(self, reports: List[QualityReport]):
        """
        **Feature: udl-rating-framework, Property 31: Statistical Significance Testing**
        ...
        for result in summary.pairwise_results:
            ...
            assert 0.0 <= result.ttest_pvalue <= 1.0, f"Invalid t-test p-value: {result.ttest_pvalue}"
            ...
            if result.wilcoxon_pvalue is not None:
                ...
                assert 0.0 <= result.wilcoxon_pvalue <= 1.0, f"Invalid Wilcoxon p-value: {result.wilcoxon_pvalue}"
...
class TestComparisonEngineUnitTests:
    ...
    def test_pairwise_comparisons(self):
        ...
        differences = {(r.udl1_name, r.udl2_name): r.difference for r in summary.pairwise_results}
    ...
    def test_ranking_generation(self):
        ...
        for ranking in rankings:
            assert isinstance(ranking.confidence_interval, tuple)
            assert len(ranking.confidence_interval) == 2
            assert ranking.confidence_interval[0] <= ranking.score <= ranking.confidence_interval[1]
    ...
    def test_confidence_interval_computation(self):
        ...
        summary = engine.compare_udls([report, report])  # Need at least 2 for comparison
