...
def test_golden_parity(
    golden_test_model_parity,
    golden_test_input_parity,
    golden_test_expected_predictions_parity,
    golden_test_expected_certainties_parity,
    ...
    (
        ...
        attention_tracking,
    ) = golden_test_model_parity(golden_test_input_parity, track=True)
...
def test_golden_qamnist(
    golden_test_model_qamnist,
    golden_test_input_qamnist,
    golden_test_expected_predictions_qamnist,
    golden_test_expected_certainties_qamnist,
    ...
    golden_test_expected_embeddings_tracking_qamnist,
):
    ...
    (
        ...
        attention_tracking,
        embedding_tracking,
    ) = golden_test_model_qamnist(x, z=z, track=True)
...
def test_golden_rl(
    golden_test_model_rl,
    golden_test_inputs_rl,
    golden_test_expected_initial_state_trace_rl,
    ...
    golden_test_expected_action_entropy_rl,
    golden_test_expected_value_rl,
    ...
    initial_state_trace, initial_activated_state_trace = (
        golden_test_model_rl.get_initial_state(num_envs=1)
    )
    ...
    (
        ...
        entropy,
        value,
        (state_trace, activated_state_trace),
        tracking_data,
        action_logits,
        action_probs,
    ) = golden_test_model_rl.get_action_and_value(
        golden_test_inputs_rl,
        (initial_state_trace, initial_activated_state_trace),
        dones,
        track=True,
...
def test_set_synchronisation_parameters(
    ...
    if neuron_select_type == "first-last":
        ...
        assert torch.equal(
            right[:num_random_pairing_self], left[:num_random_pairing_self]
        )
...
@pytest.mark.parametrize(
    "backbone_type, positional_embedding_type",
    list(itertools.product(VALID_BACKBONE_TYPES, VALID_POSITIONAL_EMBEDDING_TYPES)),
)
def test_valid_backbone_and_valid_positional_embedding(
...
def test_none_backbone_with_valid_positional_embeddings(
...
def test_valid_backbone_with_none_positional_embeddings(
...
def test_qamnist_prediction_shape(
...
def test_qamnist_synchronisation_shape(
