    ...
    def visit_Call(self, node: ast.Call):
        ...
        if (isinstance(node.func, ast.Attribute) and 
            ...
            if node.args and isinstance(node.args[0], ast.Constant):
                self.env_vars.append((node.args[0].value, node.lineno, self.current_file))
...
def extract_documented_env_vars(project_root: Path) -> Set[str]:
    ...
    for doc_file in doc_files:
        ...
        code_pattern = r'(?:os\.getenv|os\.environ\.get)\s*\(\s*["\']([A-Z][A-Z0-9_]+)["\']'
...
def get_deployment_env_vars(project_root: Path) -> Dict[str, List[Tuple[int, str]]]:
    ...
    for var_name, locations in all_vars.items():
        ...
        deployment_locations = [
            (line, path) for line, path in locations 
...
class TestEnvVarDocumentation:
    ...
    def test_all_deployment_env_vars_documented(self):
        ...
        *For any* environment variable read by the deployment application,
        ...
        if undocumented:
            msg_lines = ["The following environment variables are used but not documented:"]
            for item in undocumented:
                msg_lines.append(f"  - {item['variable']} (used in {item['file']}:{item['line']})")
...
'''
    ...
    def test_documentation_extraction_works(self):
        ...
        expected_vars = {'API_TOKEN', 'CTM_MODEL_PATH', 'ENVIRONMENT', 'PORT', 'LOG_LEVEL'}
        
        for var in expected_vars:
            assert var in self.documented_vars, f"Expected {var} to be documented in deployment docs"


@given(st.sampled_from(list(get_deployment_env_vars(get_project_root()).keys()) or ['PLACEHOLDER']))
