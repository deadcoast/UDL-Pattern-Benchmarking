...
class TestIDEPluginManager:
    ...
    def test_vscode_extension_generation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert 'udl-rating.checkQuality' in [cmd['command'] for cmd in package_data['contributes']['commands']]
    ...
    def test_intellij_plugin_generation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (plugin_dir / 'src' / 'main' / 'java' / 'com' / 'udlrating' / 'intellij' / 'UDLFileType.java').exists()
...
class TestIntegrationCLI:
    ...
    def test_git_hooks_cli_integration(self, mock_manager_class):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            result = runner.invoke(install_git_hooks, [
                '--repo-path', temp_dir,
                '--threshold', '0.8'
            ])
