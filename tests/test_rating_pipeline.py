...
from udl_rating_framework.core.pipeline import RatingPipeline, QualityReport, ComputationStep
    ...
    **Validates: Requirements 5.1**
    ...
    def test_metrics_computed_independently(self, grammar_rules):
        ...
        for metric_name in all_metrics:
            try:
                ...
                report = pipeline.compute_rating(udl)
                individual_results[metric_name] = report.metric_scores.get(metric_name, 0.0)
            except Exception:
    ...
    def test_metric_state_isolation(self):
        ...
        for metric_name in ["consistency", "completeness"]:
            if (metric_name in report1_first.metric_scores and 
                metric_name in report1_second.metric_scores):
    ...
    **Validates: Requirements 2.4**
    ...
    def test_all_results_included_in_summary(self, udl_contents_list):
        ...
        for i, report in enumerate(reports):
            ...
            assert hasattr(report, 'overall_score'), \
                f"Report {i} missing overall_score"
            
            assert hasattr(report, 'confidence'), \
                f"Report {i} missing confidence"
            
            assert hasattr(report, 'metric_scores'), \
                f"Report {i} missing metric_scores"
            
            assert hasattr(report, 'udl_file'), \
                f"Report {i} missing udl_file"
    ...
    def test_aggregation_preserves_individual_results(self):
        ...
        for i, (individual, batch) in enumerate(zip(individual_reports, batch_reports)):
            ...
            for metric_name in ["consistency", "completeness"]:
                if (metric_name in individual.metric_scores and 
                    metric_name in batch.metric_scores):
...
class TestRatingPipelineUnits:
    ...
    def test_report_generation_completeness(self):
        ...
        for config in configurations:
            ...
            required_fields = [
                'overall_score', 'confidence', 'metric_scores', 'metric_formulas',
                'computation_trace', 'error_bounds', 'timestamp', 'udl_file',
                'errors', 'warnings'
            ]
    ...
    def test_pipeline_validation_functionality(self):
        ...
        expected_keys = [
            "consistency_computable", "consistency_bounded",
            "completeness_computable", "completeness_bounded",
            "aggregation_works", "aggregation_bounded",
            "confidence_works", "confidence_bounded",
            "pipeline_works", "report_complete"
        ]
        ...
        for key in expected_keys:
            ...
            assert value in [True, False] or bool(value) in [True, False], f"Validation key {key} should be boolean-like"
