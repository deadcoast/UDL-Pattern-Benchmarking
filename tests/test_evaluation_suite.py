...
from udl_rating_framework.evaluation.evaluation_suite import EvaluationSuite, EvaluationResult
...
class TestEvaluationSuite:
    ...
    def test_initialization(self):
        ...
        suite = EvaluationSuite(k_folds=10, bootstrap_samples=2000, confidence_level=0.99)
    ...
    def test_property_correlation_reporting(self, n_samples, noise_level):
        ...
        **Validates: Requirements 10.2**
        ...
        pearson_r, pearson_ci, spearman_r, spearman_ci = self.suite.compute_correlations_with_ci(y_true, y_pred)
        
        # Verify Pearson correlation is computed
        expected_pearson, _ = pearsonr(y_true, y_pred)
        assert abs(pearson_r - expected_pearson) < 1e-10, f"Pearson correlation mismatch: {pearson_r} vs {expected_pearson}"
        
        # Verify Spearman correlation is computed
        expected_spearman, _ = spearmanr(y_true, y_pred)
        assert abs(spearman_r - expected_spearman) < 1e-10, f"Spearman correlation mismatch: {spearman_r} vs {expected_spearman}"
        ...
        assert pearson_ci[0] <= pearson_ci[1], f"Pearson CI bounds invalid: {pearson_ci}"
        assert spearman_ci[0] <= spearman_ci[1], f"Spearman CI bounds invalid: {spearman_ci}"
        
        # Verify correlations are within their CIs (should be true for bootstrap)
        assert pearson_ci[0] <= pearson_r <= pearson_ci[1], f"Pearson r {pearson_r} not in CI {pearson_ci}"
        assert spearman_ci[0] <= spearman_r <= spearman_ci[1], f"Spearman r {spearman_r} not in CI {spearman_ci}"
    ...
    def test_property_calibration_error_computation(self, n_samples, n_bins):
        ...
        **Validates: Requirements 10.3**
        ...
        ece = self.suite.compute_calibration_error(y_true, y_pred, confidences, n_bins=n_bins)
        ...
        perfect_ece = self.suite.compute_calibration_error(y_true, y_pred, perfect_confidences, n_bins=n_bins)
        
        # Perfect calibration should have low ECE (though not necessarily 0 due to binning)
        assert perfect_ece >= 0, f"Perfect calibration ECE should be non-negative, got {perfect_ece}"
    ...
    def test_property_error_distribution_analysis(self, n_samples, error_scale):
        ...
        **Validates: Requirements 10.4**
        ...
        assert isinstance(shapiro_stat, (int, float)), f"Shapiro statistic should be numeric, got {type(shapiro_stat)}"
        assert isinstance(p_value, (int, float)), f"P-value should be numeric, got {type(p_value)}"
        
        # Verify test statistic is in valid range
        assert 0 <= shapiro_stat <= 1, f"Shapiro statistic should be in [0,1], got {shapiro_stat}"
    ...
    def test_property_bootstrap_confidence_intervals(self, n_samples, noise_level):
        ...
        **Validates: Requirements 10.5**
        ...
        assert self.suite.bootstrap_samples >= 1000, f"Bootstrap samples should be ≥ 1000, got {self.suite.bootstrap_samples}"
        ...
        for metric in expected_metrics:
            ...
            assert isinstance(ci, tuple) and len(ci) == 2, f"CI for {metric} should be tuple of length 2"
        ...
        if not (np.isnan(rmse_ci[0]) or np.isnan(rmse_ci[1])):
            assert rmse_ci[0] >= 0, f"RMSE lower bound should be non-negative: {rmse_ci}"
...
class TestEvaluationSuiteIntegration:
    ...
    def test_cross_validation_execution(self):
        ...
        cv_scores, mean_score, std_score = self.suite.k_fold_cross_validation(X, y, model_fn)
        
        # Verify results
        assert len(cv_scores) == 5, f"Should have 5 CV scores, got {len(cv_scores)}"
        assert all(isinstance(score, (int, float)) for score in cv_scores), "All CV scores should be numeric"
    ...
    def test_report_generation(self):
        ...
        assert -1 <= result.pearson_correlation <= 1, "Pearson correlation should be in [-1, 1]"
        assert -1 <= result.spearman_correlation <= 1, "Spearman correlation should be in [-1, 1]"
        assert 0 <= result.calibration_error <= 1, "Calibration error should be in [0, 1]"
        assert 0 <= result.shapiro_statistic <= 1, "Shapiro statistic should be in [0, 1]"
        ...
        for metric_name, (lower, upper) in result.bootstrap_metrics.items():
            if not (np.isnan(lower) or np.isnan(upper)):
                assert lower <= upper, f"Bootstrap CI bounds invalid for {metric_name}: ({lower}, {upper})"