...
from udl_rating_framework.core.performance import (
    PerformanceOptimizer, PerformanceConfig, ProcessingStrategy,
    process_files_optimized, benchmark_all_strategies
)

# Import performance modules for testing
from udl_rating_framework.core.streaming import (
    StreamingProcessor, StreamingConfig, process_large_file
)
from udl_rating_framework.core.memory_mapping import (
    MemoryMappedUDLProcessor, MemoryMappedFile, process_large_udl_file
)
from udl_rating_framework.core.incremental import (
    IncrementalProcessor, FileSnapshot, process_udl_incremental
)
...
try:
    from udl_rating_framework.core.distributed import (
        DistributedProcessor, process_files_distributed
    )
...
class TestStreamingProcessor:
    ...
    def test_process_large_file_convenience(self, large_temp_file):
        ...
        results = list(process_large_file(
        ...
        assert len(results) > 0
        assert all(isinstance(r.aggregated_score, float) for r in results if not r.error)
...
@pytest.mark.skipif(not DISTRIBUTED_AVAILABLE, reason="Distributed computing not available")
...
class TestIntegration:
    ...
    def test_performance_with_different_file_sizes(self, sample_udl_content, large_udl_content):
