...
from udl_rating_framework.models.ctm_adapter import UDLRatingCTM, UDLTokenVocabulary, create_udl_rating_model, TrackingData
...
class TestCTMAdapterProperties:
    ...
    def test_property_synchronization_extraction(self, vocab_size, iterations, batch_size, seq_len):
        ...
        **Validates: Requirements 5.2**
        ...
        assert tracking_data.synch_out.shape == (iterations, batch_size, model.get_synch_out_dim())
    ...
    def test_property_activation_recording(self, vocab_size, iterations, batch_size, seq_len):
        """
        ...
        **Validates: Requirements 7.1**
        ...
        assert tracking_data.pre_activations.shape[2] == tracking_data.n_neurons  # All neurons
        ...
        assert tracking_data.post_activations.shape[1] == batch_size  # All batch samples
        assert tracking_data.post_activations.shape[2] == tracking_data.n_neurons  # All neurons
    ...
    @settings(max_examples=20, deadline=60000)
    def test_property_synchronization_matrix_recording(self, vocab_size, iterations, batch_size, seq_len):
        """
        ...
        **Validates: Requirements 7.2**
        ...
        assert tracking_data.synch_out.shape == (iterations, batch_size, tracking_data.n_synch_out)
        assert tracking_data.synch_action.shape == (iterations, batch_size, tracking_data.n_synch_action)
        ...
        for t in range(iterations):
            ...
            assert not np.isnan(tracking_data.synch_out[t]).any(), f"NaN found in synch_out at iteration {t}"
            assert not np.isnan(tracking_data.synch_action[t]).any(), f"NaN found in synch_action at iteration {t}"
            assert np.isfinite(tracking_data.synch_out[t]).all(), f"Non-finite values in synch_out at iteration {t}"
            assert np.isfinite(tracking_data.synch_action[t]).all(), f"Non-finite values in synch_action at iteration {t}"
        ...
        for metric_name, metric_value in evolution_metrics.items():
            assert np.isfinite(metric_value), f"Non-finite evolution metric: {metric_name} = {metric_value}"
    
    @given(
        vocab_size=st.integers(min_value=10, max_value=100),
        iterations=st.integers(min_value=2, max_value=8),
        batch_size=st.integers(min_value=1, max_value=4),
        seq_len=st.integers(min_value=2, max_value=15),  # At least 2 for meaningful attention
        heads=st.integers(min_value=1, max_value=4)
    )
    @settings(max_examples=20, deadline=60000)
    def test_property_attention_weight_normalization(self, vocab_size, iterations, batch_size, seq_len, heads):
        """
        ...
        **Validates: Requirements 7.3**
        ...
        if len(actual_shape) == 5:
            ...
            attention_weights_reshaped = tracking_data.attention_weights.squeeze(3)  # Remove the extra dimension
        else:
        ...
        attention_sums = np.sum(attention_weights_reshaped, axis=-1)  # [iterations, batch, heads]
        ...
        assert (attention_weights_reshaped >= 0).all(), "Found negative attention weights"
        assert np.isfinite(attention_weights_reshaped).all(), "Found non-finite attention weights"
        ...
        for t in range(iterations):
            for b in range(batch_size):
                for h in range(heads):
                    ...
                    assert attention_sum >= 0, f"Attention sum is negative at iteration {t}, batch {b}, head {h}: sum = {attention_sum}"
                    assert attention_sum < 10.0, f"Attention sum is too large at iteration {t}, batch {b}, head {h}: sum = {attention_sum}"
        ...
        assert total_attention > 0, "All attention weights are zero across all iterations, batches, and heads"
...
class TestCTMAdapterUnitTests:
    ...
    def test_tracking_mode_enables_recording(self):
        ...
        ratings1, certainties1, synch_out1, tracking_data1 = model(token_ids, track=False)
        assert tracking_data1 is None
        
        # Test with tracking
        ratings2, certainties2, synch_out2, tracking_data2 = model(token_ids, track=True)
    ...
    def test_data_export_to_numpy_hdf5(self):
        ...
        try:
            ...
            # Verify arrays match
            np.testing.assert_array_equal(loaded_data.pre_activations, tracking_data.pre_activations)
            np.testing.assert_array_equal(loaded_data.post_activations, tracking_data.post_activations)
            np.testing.assert_array_equal(loaded_data.synch_out, tracking_data.synch_out)
            np.testing.assert_array_equal(loaded_data.synch_action, tracking_data.synch_action)
            np.testing.assert_array_equal(loaded_data.attention_weights, tracking_data.attention_weights)
    ...
    def test_visualization_generation(self):
        ...
        from udl_rating_framework.visualization import ActivationVisualizer, SynchronizationVisualizer
        ...
        fig1 = activation_viz.plot_activation_heatmap(tracking_data, 'post', batch_idx=0)
        assert isinstance(fig1, plt.Figure)
        plt.close(fig1)
        
        # Test time series generation
        fig2 = activation_viz.plot_activation_time_series(tracking_data, neuron_indices=[0, 1], activation_type='post')
        ...
        fig4 = sync_viz.plot_synchronization_evolution(tracking_data, 'out', batch_idx=0)
        assert isinstance(fig4, plt.Figure)
        plt.close(fig4)
        
        # Test synchronization time series
        fig5 = sync_viz.plot_synchronization_time_series(tracking_data, synch_indices=[0, 1], synch_type='out')
        ...
        fig7 = sync_viz.plot_attention_weights_evolution(tracking_data, head_idx=0, batch_idx=0)
    ...
    def test_synchronization_evolution_metrics(self):
        ...
        required_metrics = [
            'out_temporal_stability', 'action_temporal_stability',
            'out_convergence_distance', 'action_convergence_distance',
            'out_mean_change_rate', 'action_mean_change_rate',
            'out_final_change_rate', 'action_final_change_rate'
        ]
