    ...
    def test_metric_computation_time_vs_udl_size(self):
        ...
        for metric_name in metric_names:
            ...
            for size in udl_sizes:
                ...
                assert 0.0 <= value <= 1.0, f"Metric {metric_name} out of bounds: {value}"
        ...
        for metric_name, times in results.items():
            max_time = max(times)
            assert max_time < 10.0, f"Metric {metric_name} too slow: {max_time:.4f}s for largest input"
            
            # Verify performance doesn't degrade exponentially
            if len(times) >= 3:
                # Check that time doesn't grow faster than O(n^2)
                size_ratios = [udl_sizes[i] / udl_sizes[i-1] for i in range(1, len(udl_sizes))]
    ...
    def test_verify_complexity_bounds_on_or_ologn(self):
        ...
        for metric_name in metric_names:
            ...
            if len(times) >= 3:
                ...
                print(f"  Complexity bound: {'PASS' if avg_growth_ratio <= 3.0 else 'FAIL'}")
...
class TestCTMInferencePerformance:
    ...
    def test_ctm_inference_time_vs_sequence_length(self):
        ...
        for seq_len in sequence_lengths:
            ...
            print(f"Sequence length {seq_len}: {median_time:.4f}s ({throughput:.0f} tokens/s)")
        
        # Verify reasonable performance
        max_time = max(times)
        assert max_time < 5.0, f"CTM inference too slow: {max_time:.4f}s for largest sequence"
        ...
        if len(times) >= 3:
            ...
            print(f"  Average growth ratio: {avg_growth_ratio:.2f}")
            print(f"  Scaling: {'GOOD' if avg_growth_ratio <= 2.0 else 'ACCEPTABLE' if avg_growth_ratio <= 2.5 else 'POOR'}")
    ...
    def test_ctm_batch_processing_efficiency(self):
        ...
        for batch_size in batch_sizes:
            ...
            print(f"Batch size {batch_size}: {median_time:.4f}s total, {time_per_sample:.4f}s per sample ({throughput:.1f} samples/s)")
    ...
    def test_batch_processing_throughput(self):
        ...
        for file_count in file_counts:
            ...
            for worker_count in worker_counts:
                ...
                print(f"Files: {file_count}, Workers: {worker_count}, Time: {total_time:.3f}s, Throughput: {throughput:.2f} files/s")
                
                # Verify reasonable performance
                assert total_time < 30.0, f"Batch processing too slow: {total_time:.3f}s for {file_count} files"
        ...
        if len(memory_usages) >= 2:
            for i in range(1, len(file_counts)):
                ...
                if memory_usages[i-1] < 0.1 or memory_usages[i] < 0.1:
                    print(f"  Skipping memory ratio check for small usage: {memory_usages[i-1]:.1f} -> {memory_usages[i]:.1f} MB")
                    continue
...
class TestPerformanceBenchmarkIntegration:
    ...
    def test_benchmark_metric_computation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(f"Benchmark completed: {summary['total_benchmarks']} tests, "
    ...
    def test_benchmark_ctm_inference(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(f"CTM benchmark completed: {summary['total_benchmarks']} tests, "
                  f"avg time: {summary['avg_execution_time']:.4f}s")
    ...
    def test_verify_complexity_bounds(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for component, passed in results.items():
                ...
                print(f"Complexity check for {component}: {'PASS' if passed else 'FAIL'}")
            ...
            assert pass_rate >= 0.5, f"Too many complexity check failures: {pass_rate:.2f} pass rate"