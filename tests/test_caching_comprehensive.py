...
from udl_rating_framework.core.caching import (
    LRUCache, UDLRepresentationCache, MetricCache, CacheEntry,
    get_udl_cache, get_metric_cache, clear_all_caches
)
...
class TestUDLRepresentationCache:
    ...
    def test_cache_content_based_udl(self, mock_udl_representation):
        ...
        different_key = cache._get_cache_key(Path("virtual_file.udl"), content=different_content)
        assert different_key != cache_key
        assert different_key.startswith("content:")
    
    def test_cache_invalidation_on_file_change(self, temp_udl_file, mock_udl_representation):
    ...
    def test_cache_ttl_expiration(self, temp_udl_file, mock_udl_representation):
        ...
        with cache.cache._lock:
            if cache_key in cache.cache._cache:
                ...
                cache.cache._cache[cache_key].created_at = datetime.now() - timedelta(hours=1)
    ...
    def test_cache_performance_under_high_load(self):
        ...
        assert operation_time < 5.0, f"Cache operations took too long: {operation_time}s"
...
class TestCacheIntegration:
    ...
    def test_concurrent_cache_access_with_evictions(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            futures = [
                executor.submit(worker_with_evictions, i)
                for i in range(3)
            ]
            concurrent.futures.wait(futures)
