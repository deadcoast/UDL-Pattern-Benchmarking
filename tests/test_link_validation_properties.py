...
def _import_link_validator():
    ...
    module_path = Path(__file__).parent.parent / "udl_rating_framework" / "validation" / "link_validator.py"
...
class TestPropertyBasedLinkValidation:
    ...
    **Validates: Requirements 2.2, 2.3, 2.4**
    ...
    @given(st.text(alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-", min_size=1, max_size=20))
        ...
        **Validates: Requirements 2.2**
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if links:  # Only test if link was extracted
                result = validator.validate_file_link(links[0])
                assert result.is_valid is True, f"Valid file {safe_filename} should validate"
    ...
    def test_nonexistent_file_always_fails(self, filename):
        ...
        **Validates: Requirements 2.2, 2.4**
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if links:
                result = validator.validate_file_link(links[0])
                assert result.is_valid is False, f"Nonexistent file {safe_filename} should fail"
