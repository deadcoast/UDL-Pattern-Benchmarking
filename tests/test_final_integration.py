...
class TestFinalIntegration:
    ...
    def test_end_to_end_rating_pipeline(self, temp_dir):
        ...
        for udl_file in test_files:
            try:
                ...
                expected_metrics = {'consistency', 'completeness', 'expressiveness', 'structural_coherence'}
                ...
                for metric_name, score in report.metric_scores.items():
                    assert 0 <= score <= 1, f"Metric {metric_name} score {score} not in [0,1]"
    ...
    def test_error_recovery_scenarios(self, temp_dir):
        ...
        print(f"Successfully processed {successful_ratings}/{len(error_files)} error scenario files")
    ...
    def test_various_udl_formats_and_sizes(self, temp_dir):
        ...
        for filename, content in test_cases:
            ...
            try:
                ...
                for metric_name, score in report.metric_scores.items():
                    assert 0 <= score <= 1, f"File {filename}, metric {metric_name}: score {score} not in [0,1]"
                
                successful_tests += 1
                print(f"✓ Successfully processed {filename} (score: {report.overall_score:.3f})")
        ...
        assert success_rate >= 0.8, f"Only {successful_tests}/{len(test_cases)} test cases succeeded"
        
        print(f"Successfully processed {successful_tests}/{len(test_cases)} UDL format variations")
    ...
    def test_mathematical_correctness_verification(self, temp_dir):
        ...
        print(f"✓ Mathematical correctness verified on {len(udl_files[:10])} UDL examples")
    ...
    def test_performance_and_scalability(self, temp_dir):
        ...
        for size_name, num_rules in test_cases:
            ...
            try:
                ...
                print(f"✓ {size_name} ({num_rules} rules): {processing_time:.3f}s, score: {report.overall_score:.3f}")
        ...
        assert len(successful_cases) >= 2, f"Only {len(successful_cases)} performance test cases succeeded"
        
        print(f"✓ Performance testing completed: {len(successful_cases)}/{len(test_cases)} cases successful")
