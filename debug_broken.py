...
"""
    ...
    try:
        ...
        print(f"✓ Grammar graph: {graph.number_of_nodes()} nodes, {graph.number_of_edges()} edges")
        ...
        if contradictions:
            for c1, c2 in contradictions:
                print(f"  Contradiction: {c1.lhs} ::= {' '.join(c1.rhs)} vs {c2.lhs} ::= {' '.join(c2.rhs)}")
        ...
        print(f"✓ Manual calculation: 1 - ({num_contradictions} + {num_cycles}) / ({num_rules} + 1) = {manual_score:.6f}")
