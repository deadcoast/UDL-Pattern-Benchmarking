...
class CoverageMonitor:
            ...
            if line.strip() and not line.startswith('-') and not line.startswith('Name'):
                ...
                if len(parts) >= 4:
                    try:
                        ...
                        missing = int(parts[2])
                        coverage = float(parts[3][:-1]) if parts[3].endswith('%') else 0.0
    ...
    def check_coverage_thresholds(self, report: CoverageReport) -> List[str]:
        ...
        if report.overall.coverage < self.critical_threshold:
            alerts.append(f"🚨 CRITICAL: Overall coverage {report.overall.coverage:.1f}% is below critical threshold {self.critical_threshold}%")
        elif report.overall.coverage < self.threshold:
            alerts.append(f"⚠️ WARNING: Overall coverage {report.overall.coverage:.1f}% is below target threshold {self.threshold}%")
        ...
        for module in report.modules:
            if any(module.name.startswith(critical) for critical in critical_modules):
                if module.coverage < 95.0:
                    alerts.append(f"⚠️ Critical module {module.name} has {module.coverage:.1f}% coverage (target: 95%)")
        
        # Check for significant drops
        previous_coverage = self.get_previous_coverage()
        if previous_coverage and report.overall.coverage < previous_coverage - 5.0:
            alerts.append(f"📉 Significant coverage drop: {previous_coverage:.1f}% → {report.overall.coverage:.1f}%")
    ...
    def generate_coverage_report(self, report: CoverageReport) -> str:
        ...
        if below_threshold:
            ...
            for module in below_threshold:
                lines.append(f"- **{module.name}:** {module.coverage:.1f}% ({module.missing} missing)")
            lines.append("")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="UDL Rating Framework Coverage Monitor")
    parser.add_argument("--threshold", type=float, default=90.0,
                       help="Coverage threshold for warnings (default: 90.0)")
    parser.add_argument("--critical-threshold", type=float, default=85.0,
                       help="Critical coverage threshold (default: 85.0)")
    parser.add_argument("--db-path", default="coverage_history.db",
