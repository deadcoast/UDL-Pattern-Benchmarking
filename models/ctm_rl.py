    ...
    def set_backbone(self):
        ...
        if self.backbone_type == "navigation-backbone":
            ...
            raise NotImplemented(
                "The only backbone supported for RL are for navigation (symbolic C x H x W inputs) and classic control (vectors of length D)."
            )
        pass
    ...
    def set_synchronisation_parameters(
        ...
        if synch_type == "action":
            ...
            left, right = self.initialize_left_right_neurons(
                "out", self.d_model, n_synch, n_random_pairing_self
            )
            ...
            self.register_parameter(
                f"decay_params_out",
                nn.Parameter(
                    torch.zeros(self.synch_representation_size_out), requires_grad=True
                ),
            )
            pass
        else:
