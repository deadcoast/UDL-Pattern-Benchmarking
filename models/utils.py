...
def compute_decay(T, params, clamp_lims=(0, 15)):
    ...
    indices = (
        torch.arange(T - 1, -1, -1, device=params.device)
        .reshape(T, 1)
...
def add_coord_dim(x, scaled=True):
    ...
    x_coords = torch.arange(W, device=x.device, dtype=x.dtype).repeat(
        H, 1
    )  # Shape (H, W)
    y_coords = (
...
def get_latest_checkpoint_file(filepath, limit=300000):
    ...
    checkpoint_files = [
        ...
        if int(re.search(r"checkpoint_(\d+)\.pt", f).group(1)) <= limit
    ]
