...
class LSTMBaseline(nn.Module):
    ...
    def get_kv_for_step(
        ...
        if is_digit_step:
            ...
            if prev_input is not None and torch.equal(current_input, prev_input):
                return prev_kv, prev_input
            is_index_step, is_operator_step = thought_steps.determine_answer_step_type(
                stepi
            )
            if is_index_step:
    ...
    def forward(self, x, z, track=False):
        ...
        hidden_state = torch.repeat_interleave(
            self.start_hidden_state.unsqueeze(0), x.size(0), 0
        )
        cell_state = torch.repeat_interleave(
            self.start_cell_state.unsqueeze(0), x.size(0), 0
        )
        ...
        predictions = torch.empty(
            B,
            self.out_dims,
            thought_steps.total_iterations,
            device=device,
            dtype=x.dtype,
        )  # Adjust dtype if needed
        certainties = torch.empty(
            B, 2, thought_steps.total_iterations, device=device, dtype=x.dtype
        )  # Adjust dtype if needed
        ...
        for stepi in range(thought_steps.total_iterations):
            ...
            kv, prev_input = self.get_kv_for_step(
                stepi, x, z, thought_steps, prev_input, prev_kv
            )
            prev_kv = kv
