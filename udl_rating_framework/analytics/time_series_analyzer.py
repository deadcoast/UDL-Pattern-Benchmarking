...
class TimeSeriesAnalyzer:
    ...
    def analyze_quality_evolution(self, 
                                reports: List[QualityReport],
                                udl_file: str,
                                metric_name: str = 'overall_score') -> QualityEvolution:
        """
        ...
        if len(file_reports) < self.min_observations:
            raise ValueError(f"Insufficient data: need at least {self.min_observations} observations, got {len(file_reports)}")
    ...
    def analyze_multiple_metrics(self, 
                                reports: List[QualityReport],
                                udl_file: str,
                                metrics: List[str] = None) -> Dict[str, QualityEvolution]:
        """
        ...
        if metrics is None:
            ...
            metrics = ['overall_score', 'confidence'] + list(file_reports[0].metric_scores.keys())
    ...
    def _analyze_trends(self, time_series: pd.Series) -> TimeSeriesMetrics:
        ...
        timestamps_numeric = np.array([(t - time_series.index[0]).total_seconds() 
    ...
    def _detect_anomalies(self, time_series: pd.Series) -> List[Tuple[datetime, float, str]]:
        ...
        if std_val > 0:
            ...
            for i, (timestamp, value, z_score) in enumerate(zip(time_series.index, values, z_scores)):
                if z_score > self.anomaly_threshold:
                    reason = f"Z-score: {z_score:.2f} (threshold: {self.anomaly_threshold})"
                    anomalies.append((timestamp, value, reason))
        
        # Peak detection for sudden spikes
        peaks, properties = find_peaks(values, height=mean_val + 2*std_val, distance=3)
        ...
        valleys, properties = find_peaks(-values, height=-(mean_val - 2*std_val), distance=3)
    ...
    def _forecast_quality(self, 
                         time_series: pd.Series, 
                         forecast_periods: int = 10) -> Tuple[Optional[pd.Series], Optional[Tuple[pd.Series, pd.Series]]]:
        ...
        time_delta = (time_series.index[-1] - time_series.index[-2]) if len(time_series) > 1 else timedelta(days=1)
        
        future_timestamps = [last_timestamp + (i + 1) * time_delta for i in range(forecast_periods)]
        future_timestamps_numeric = np.array([(t - time_series.index[0]).total_seconds() 
    ...
    def _generate_evolution_summary(self, 
                                  time_series: pd.Series, 
                                  trend_analysis: TimeSeriesMetrics) -> str:
        ...
        if trend_analysis.trend_p_value < 0.05:
            if trend_analysis.trend_slope > 0:
                summary_parts.append(f"Quality shows significant improvement over time (slope: {trend_analysis.trend_slope:.6f}, p-value: {trend_analysis.trend_p_value:.4f})")
            else:
                summary_parts.append(f"Quality shows significant decline over time (slope: {trend_analysis.trend_slope:.6f}, p-value: {trend_analysis.trend_p_value:.4f})")
        else:
            summary_parts.append(f"No significant trend detected (p-value: {trend_analysis.trend_p_value:.4f})")
        
        # Volatility
        if trend_analysis.volatility > 0.1:
            summary_parts.append(f"High volatility detected (σ: {trend_analysis.volatility:.3f})")
        elif trend_analysis.volatility > 0.05:
            summary_parts.append(f"Moderate volatility (σ: {trend_analysis.volatility:.3f})")
        else:
            summary_parts.append(f"Low volatility (σ: {trend_analysis.volatility:.3f})")
        
        # Seasonality
        if trend_analysis.seasonality_strength > 0.3:
            summary_parts.append(f"Strong seasonal patterns detected (strength: {trend_analysis.seasonality_strength:.3f})")
        elif trend_analysis.seasonality_strength > 0.1:
            summary_parts.append(f"Weak seasonal patterns (strength: {trend_analysis.seasonality_strength:.3f})")
        
        # Change points
        if trend_analysis.change_points:
            summary_parts.append(f"{len(trend_analysis.change_points)} significant change points detected")
        ...
        if abs(trend_analysis.autocorrelation) > 0.5:
            summary_parts.append(f"Strong autocorrelation (ρ: {trend_analysis.autocorrelation:.3f})")
        ...
        if current_value > mean_value + time_series.std():
            summary_parts.append(f"Current quality ({current_value:.3f}) is above historical average")
        elif current_value < mean_value - time_series.std():
            summary_parts.append(f"Current quality ({current_value:.3f}) is below historical average")
        else:
            summary_parts.append(f"Current quality ({current_value:.3f}) is near historical average ({mean_value:.3f})")
        
        return ". ".join(summary_parts) + "."
    
    def generate_time_series_report(self, 
                                  evolution_results: Dict[str, QualityEvolution],
                                  udl_file: str) -> str:
        ...
        for metric_name, evolution in evolution_results.items():
            ...
            if evolution.trend_analysis.anomalies:
                ...
                for timestamp, value, reason in evolution.trend_analysis.anomalies:
                    report_lines.append(f"- {timestamp.strftime('%Y-%m-%d %H:%M:%S')}: {value:.3f} ({reason})")
                report_lines.append("")
        ...
        if overall_evolution:
            if overall_evolution.trend_analysis.trend_p_value < 0.05:
                if overall_evolution.trend_analysis.trend_slope > 0:
                    report_lines.append("✅ Quality is improving significantly. Continue current practices.")
                else:
                    report_lines.append("⚠️ Quality is declining significantly. Review recent changes and consider corrective actions.")
            else:
                report_lines.append("ℹ️ Quality is stable. Monitor for emerging trends.")
            
            if overall_evolution.trend_analysis.volatility > 0.1:
                report_lines.append("⚠️ High volatility detected. Consider implementing more consistent development practices.")
            
            if len(overall_evolution.trend_analysis.anomalies) > 0:
                report_lines.append("🔍 Anomalies detected. Investigate the causes of unusual quality changes.")
