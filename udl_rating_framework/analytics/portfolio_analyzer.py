...
class PortfolioAnalyzer:
    ...
    def analyze_portfolio(self, 
                         ...
                         project_mapping: Optional[Dict[str, str]] = None) -> PortfolioComparison:
        """
        ...
        filtered_projects = {
            project: reports_list for project, reports_list in project_reports.items()
            if len(reports_list) >= self.min_reports_per_project
        }
        
        if len(filtered_projects) < 2:
            raise ValueError(f"Need at least 2 projects with {self.min_reports_per_project}+ reports each")
        ...
        recommendations = self._generate_recommendations(project_profiles, clusters, rankings)
    ...
    def benchmark_against_standards(self, 
                                  portfolio_comparison: PortfolioComparison,
                                  industry_benchmarks: Optional[Dict[str, float]] = None) -> BenchmarkAnalysis:
        """
            ...
            for profile in profiles.values():
                for metric in profile.metric_averages:
                    if metric not in industry_standards:
                        metric_values = [p.metric_averages.get(metric, 0) for p in profiles.values()]
        ...
        for project, profile in profiles.items():
            ...
            # Overall score gap
            gaps['overall_score'] = industry_standards['overall_score'] - profile.avg_overall_score
            
            # Confidence gap
            gaps['confidence'] = industry_standards['confidence'] - profile.avg_confidence
            
            # Consistency gap
            gaps['consistency_score'] = industry_standards['consistency_score'] - profile.consistency_score
            ...
            improvement_potential[project] = np.mean(positive_gaps) if positive_gaps else 0.0
    ...
    def _group_reports_by_project(self, 
                                reports: List[QualityReport],
                                project_mapping: Optional[Dict[str, str]]) -> Dict[str, List[QualityReport]]:
        """Group reports by project."""
        project_reports = {}
    ...
    def _create_project_profile(self, 
                              project_name: str, 
                              reports: List[QualityReport]) -> ProjectProfile:
        ...
        if len(reports) > 1:
            ...
            timestamps = [(r.timestamp - sorted_reports[0].timestamp).total_seconds() 
        ...
        risk_level = self._assess_risk_level(
            avg_overall_score, std_overall_score, quality_trend, len([r for r in reports if r.errors])
        )
    ...
    def _assess_maturity_level(self, 
                             avg_score: float, 
                             consistency: float, 
                             trend: float, 
                             num_reports: int) -> str:
    ...
    def _generate_rankings(self, profiles: Dict[str, ProjectProfile]) -> Dict[str, List[str]]:
        ...
        rankings['overall_score'] = sorted(
            profiles.keys(), 
            key=lambda p: profiles[p].avg_overall_score, 
            reverse=True
        )
        ...
        rankings['confidence'] = sorted(
            profiles.keys(), 
            key=lambda p: profiles[p].avg_confidence, 
            reverse=True
        )
        ...
        rankings['consistency'] = sorted(
            profiles.keys(), 
            key=lambda p: profiles[p].consistency_score, 
            reverse=True
        )
        ...
        rankings['improvement'] = sorted(
            profiles.keys(), 
            key=lambda p: profiles[p].quality_trend, 
            reverse=True
        )
        ...
        for metric in all_metrics:
            ...
            projects_with_metric = [
                ...
                if metric in profiles[p].metric_averages
            ]
    ...
    def _perform_cluster_analysis(self, profiles: Dict[str, ProjectProfile]) -> Dict[str, List[str]]:
    ...
    def _compute_project_correlations(self, profiles: Dict[str, ProjectProfile]) -> pd.DataFrame:
        ...
        projects = list(profiles.keys())
        feature_names = [
            'avg_overall_score', 'avg_confidence', 'consistency_score', 
            'quality_trend', 'num_udl_files', 'total_reports'
        ]
    ...
    def _identify_outliers(self, profiles: Dict[str, ProjectProfile]) -> List[str]:
        ...
        for values, metric_name in [(overall_scores, 'overall'), 
    ...
    def _generate_recommendations(self, 
                                ...
                                rankings: Dict[str, List[str]]) -> Dict[str, List[str]]:
        ...
        for project, profile in profiles.items():
            ...
            if profile.avg_overall_score < 0.5:
                project_recommendations.append("Focus on improving overall UDL quality - consider code reviews and quality guidelines")
            elif profile.avg_overall_score < 0.7:
                project_recommendations.append("Good quality foundation - focus on consistency and best practices")
            else:
                project_recommendations.append("Excellent quality - consider sharing best practices with other projects")
            
            # Consistency recommendations
            if profile.consistency_score < 0.6:
                project_recommendations.append("High variability in quality - implement standardized development processes")
            
            # Trend recommendations
            if profile.quality_trend < -0.001:
                project_recommendations.append("Quality is declining - investigate recent changes and implement corrective measures")
            elif profile.quality_trend > 0.001:
                project_recommendations.append("Quality is improving - continue current practices and document successful approaches")
            
            # Risk-based recommendations
            if profile.risk_level == 'high':
                project_recommendations.append("High risk project - requires immediate attention and monitoring")
            elif profile.risk_level == 'medium':
                project_recommendations.append("Medium risk - implement additional quality controls")
            
            # Maturity-based recommendations
            if profile.maturity_level == 'developing':
                project_recommendations.append("Developing project - establish quality baselines and monitoring")
            elif profile.maturity_level == 'stable':
                project_recommendations.append("Stable project - focus on continuous improvement")
            else:
                project_recommendations.append("Mature project - mentor other projects and share expertise")
            ...
            if overall_ranking > total_projects * 0.75:
                project_recommendations.append("Below average performance - study top-performing projects for improvement ideas")
            elif overall_ranking <= total_projects * 0.25:
                project_recommendations.append("Top performer - document and share successful practices")
            ...
            for cluster_name, cluster_projects in clusters.items():
                if project in cluster_projects and len(cluster_projects) > 1:
                    ...
                    project_recommendations.append(f"Similar to projects: {', '.join(other_projects)} - consider collaboration and knowledge sharing")
                    break
    ...
    def _compute_percentiles(self, profiles: Dict[str, ProjectProfile]) -> Dict[str, Dict[str, float]]:
    ...
    def generate_portfolio_report(self, 
                                portfolio_comparison: PortfolioComparison,
                                benchmark_analysis: Optional[BenchmarkAnalysis] = None) -> str:
        ...
        for cluster_name, cluster_projects in portfolio_comparison.clusters.items():
            ...
            for project in cluster_projects:
                profile = profiles[project]
                report_lines.append(f"- {project} (Quality: {profile.avg_overall_score:.3f}, Maturity: {profile.maturity_level})")
            report_lines.append("")
        ...
        if benchmark_analysis:
            ...
            for project, gaps in benchmark_analysis.performance_gaps.items():
                improvement_potential = benchmark_analysis.improvement_potential[project]
                report_lines.extend([
                    f"**{project}:**",
