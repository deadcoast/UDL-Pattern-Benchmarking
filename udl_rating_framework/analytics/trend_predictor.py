...
class TrendPredictor:
    ...
    def predict_quality_trends(self, 
                             ...
                             udl_file: str,
                             metrics: List[str] = None) -> Dict[str, PredictionResult]:
        """
        ...
        if len(file_reports) < 10:
            raise ValueError(f"Insufficient data: need at least 10 reports, got {len(file_reports)}")
    ...
    def analyze_portfolio_trends(self, 
                               reports: List[QualityReport],
                               project_mapping: Optional[Dict[str, str]] = None) -> TrendAnalysis:
        """
            ...
            try:
                ...
                for project, project_reports_list in project_reports.items():
                    if len(project_reports_list) >= 5:  # Minimum data requirement
                        project_reports_list = sorted(project_reports_list, key=lambda r: r.timestamp)
                ...
                df = pd.DataFrame(metric_data, columns=['timestamp', 'value', 'project'])
                df = df.sort_values('timestamp')
                
                # Compute historical trend
                timestamps_numeric = [(t - df['timestamp'].iloc[0]).total_seconds() 
                                    for t in df['timestamp']]
                slope, _, _, p_value, _ = stats.linregress(timestamps_numeric, df['value'])
                ...
                warnings.warn(f"Could not analyze portfolio trends for metric {metric}: {e}")
                continue
    ...
    def _predict_single_metric(self, 
                             reports: List[QualityReport], 
                             metric: str) -> PredictionResult:
        ...
        future_timestamps = [last_timestamp + timedelta(days=i+1) 
        ...
        prediction_series = pd.Series(predictions, index=pd.DatetimeIndex(future_timestamps))
        ...
        lower_bound = pd.Series(predictions - margin_of_error, index=prediction_series.index)
        upper_bound = pd.Series(predictions + margin_of_error, index=prediction_series.index)
    ...
    def _prepare_features(self, ts: pd.Series) -> Tuple[np.ndarray, np.ndarray]:
        ...
        timestamps_numeric = np.array([(t - ts.index[0]).total_seconds() / (24 * 3600) 
                                     for t in ts.index])  # Days since start
    ...
    def _prepare_future_features(self, 
                               ts: pd.Series, 
        ...
        for i, future_timestamp in enumerate(future_timestamps):
            # Time-based features
            days_since_start = (future_timestamp - ts.index[0]).total_seconds() / (24 * 3600)
            time_features = [
                days_since_start,
                days_since_start % 7,
                days_since_start % 30,
    ...
    def _train_model(self, 
                    X: np.ndarray, 
                    y: np.ndarray, 
                    model_type: str) -> Tuple[Any, Optional[StandardScaler]]:
    ...
    def _get_feature_names(self) -> List[str]:
        ...
        return [
            'time_trend', 'day_of_week', 'monthly_cycle',
            'lag_1', 'lag_2', 'lag_3', 'lag_4', 'lag_5',
            'recent_mean', 'recent_std', 'last_value'
        ]
    ...
    def _estimate_forecast_accuracy(self, df: pd.DataFrame) -> float:
        ...
        timestamps_numeric = [(t - df['timestamp'].iloc[0]).total_seconds() / (24 * 3600) 
                            for t in df['timestamp']]
    ...
    def generate_prediction_report(self, 
        ...
        if 'overall_score' in predictions:
            ...
            current_trend = (overall_pred.predictions.iloc[-1] - overall_pred.predictions.iloc[0]) / len(overall_pred.predictions)
        ...
        for metric, pred_result in predictions.items():
            ...
            if pred_result.feature_importance:
                ...
                sorted_features = sorted(pred_result.feature_importance.items(), 
        ...
        if 'overall_score' in predictions:
            ...
            current_value = overall_pred.predictions.iloc[0]  # Approximate current value
            
            if future_mean > current_value + 0.05:
                report_lines.append("✅ Quality is expected to improve significantly. Continue current practices.")
            elif future_mean < current_value - 0.05:
                report_lines.append("⚠️ Quality is expected to decline. Consider implementing corrective measures.")
            else:
                report_lines.append("ℹ️ Quality is expected to remain stable. Monitor for emerging trends.")
            
            # Model reliability assessment
            r2_score = overall_pred.model_performance.get('r2', 0.0)
            if r2_score > 0.7:
                report_lines.append("🎯 High prediction confidence - model is reliable for decision making.")
            elif r2_score > 0.4:
                report_lines.append("⚠️ Moderate prediction confidence - use predictions as guidance only.")
            else:
                report_lines.append("❌ Low prediction confidence - predictions should be interpreted cautiously.")
