...
class BusinessIntelligenceExporter:
    ...
    def __init__(self):
        ...
        self.supported_formats = [
            ...
            'powerbi', 'tableau', 'qlik', 'looker'
        ]
    
    def export_quality_data(self, 
                          reports: List[QualityReport],
                          config: BIExportConfig,
                          output_path: Optional[Path] = None,
                          project_mapping: Optional[Dict[str, str]] = None) -> BIDataset:
        """
        ...
        if config.aggregation_level == 'detailed':
            ...
            dataset = self._prepare_summary_dataset(reports, project_mapping, config.time_grouping)
        else:  # executive
    ...
    def export_dashboard_data(self, 
                            ...
                            output_path: Optional[Path] = None,
                            project_mapping: Optional[Dict[str, str]] = None) -> Dict[str, BIDataset]:
        """
        ...
        if dashboard_type == 'executive':
            ...
            datasets['portfolio_overview'] = self._create_portfolio_overview_dataset(reports, project_mapping)
            datasets['risk_assessment'] = self._create_risk_assessment_dataset(reports, project_mapping)
            
        elif dashboard_type == 'operational':
            # Operational dashboard datasets
            datasets['daily_metrics'] = self._create_daily_metrics_dataset(reports, project_mapping)
            datasets['project_status'] = self._create_project_status_dataset(reports, project_mapping)
            datasets['quality_alerts'] = self._create_quality_alerts_dataset(reports, project_mapping)
            datasets['improvement_tracking'] = self._create_improvement_tracking_dataset(reports, project_mapping)
            
        else:  # analytical
            # Analytical dashboard datasets
            datasets['detailed_metrics'] = self._create_detailed_metrics_dataset(reports, project_mapping)
            datasets['correlation_analysis'] = self._create_correlation_dataset(reports, project_mapping)
            datasets['anomaly_detection'] = self._create_anomaly_dataset(reports, project_mapping)
            datasets['predictive_analytics'] = self._create_predictive_dataset(reports, project_mapping)
    ...
    def create_powerbi_package(self, 
                             reports: List[QualityReport],
                             output_path: Path,
                             project_mapping: Optional[Dict[str, str]] = None) -> Path:
        """
        ...
        # Export data files
        datasets = self.export_dashboard_data(reports, 'executive', package_path, project_mapping)
    ...
    def create_tableau_package(self, 
                             reports: List[QualityReport],
                             output_path: Path,
                             project_mapping: Optional[Dict[str, str]] = None) -> Path:
        """
        ...
        datasets = self.export_dashboard_data(reports, 'analytical', package_path, project_mapping)
    ...
    def _prepare_detailed_dataset(self, 
                                reports: List[QualityReport],
                                project_mapping: Optional[Dict[str, str]]) -> pd.DataFrame:
    ...
    def _prepare_summary_dataset(self, 
                               reports: List[QualityReport],
                               project_mapping: Optional[Dict[str, str]],
                               time_grouping: str) -> pd.DataFrame:
        ...
        if time_grouping == 'daily':
            ...
            detailed_df['year_week'] = detailed_df['year'].astype(str) + '_W' + detailed_df['week'].astype(str)
            group_cols.append('year_week')
        elif time_grouping == 'monthly':
            detailed_df['year_month'] = detailed_df['year'].astype(str) + '_' + detailed_df['month'].astype(str).str.zfill(2)
    ...
    def _prepare_executive_dataset(self, 
                                 reports: List[QualityReport],
                                 project_mapping: Optional[Dict[str, str]]) -> pd.DataFrame:
        ...
        for project, project_reports in project_data.items():
            ...
            exec_row = {
                ...
                'data_quality': 'High' if len(project_reports) > 10 else 'Medium' if len(project_reports) > 5 else 'Low'
            }
    ...
    def _generate_metadata(self, 
                          reports: List[QualityReport],
                          config: BIExportConfig) -> Dict[str, Any]:
        ...
        return {
            ...
            'data_summary': {
                ...
                'date_range': {
                    'start': min(r.timestamp for r in reports).isoformat() if reports else None,
                    'end': max(r.timestamp for r in reports).isoformat() if reports else None
                },
                'unique_udl_files': len(set(r.udl_file for r in reports)),
                'unique_projects': len(set(r.udl_file.split('/')[0] for r in reports if '/' in r.udl_file))
            },
            'schema_version': '1.0',
    ...
    def _create_kpi_dataset(self, reports: List[QualityReport], project_mapping: Optional[Dict[str, str]]) -> BIDataset:
        """Create KPI dataset for executive dashboard."""
        # Calculate portfolio-wide KPIs
        if not reports:
            return BIDataset("kpis", "Key Performance Indicators", pd.DataFrame(), {}, datetime.now())
        
        latest_reports = {}
        for report in reports:
            key = report.udl_file
            if key not in latest_reports or report.timestamp > latest_reports[key].timestamp:
                latest_reports[key] = report
        
        kpi_data = {
            'metric': ['Portfolio Average Quality', 'Portfolio Confidence', 'Total UDL Files', 'Files with Errors', 'Quality Trend'],
            'value': [
    ...
    def _create_trend_dataset(self, reports: List[QualityReport], project_mapping: Optional[Dict[str, str]]) -> BIDataset:
    ...
    def _create_portfolio_overview_dataset(self, reports: List[QualityReport], project_mapping: Optional[Dict[str, str]]) -> BIDataset:
    ...
    def _create_risk_assessment_dataset(self, reports: List[QualityReport], project_mapping: Optional[Dict[str, str]]) -> BIDataset:
        ...
        for report in reports:
            project = report.udl_file.split('/')[0] if '/' in report.udl_file else 'default'
        ...
        for project, project_reports_list in project_reports.items():
            ...
            recent_reports = sorted(project_reports_list, key=lambda r: r.timestamp)[-10:]
            ...
            error_rate = sum(1 for r in recent_reports if r.errors) / len(recent_reports)
            if error_rate > 0.2:
                risk_score += 2
            elif error_rate > 0.1:
                risk_score += 1
    ...
    def _create_daily_metrics_dataset(self, reports: List[QualityReport], project_mapping: Optional[Dict[str, str]]) -> BIDataset:
        """Create daily metrics dataset."""
        return self._create_trend_dataset(reports, project_mapping)
    
    def _create_project_status_dataset(self, reports: List[QualityReport], project_mapping: Optional[Dict[str, str]]) -> BIDataset:
        """Create project status dataset."""
        return self._create_portfolio_overview_dataset(reports, project_mapping)
    
    def _create_quality_alerts_dataset(self, reports: List[QualityReport], project_mapping: Optional[Dict[str, str]]) -> BIDataset:
    ...
    def _create_improvement_tracking_dataset(self, reports: List[QualityReport], project_mapping: Optional[Dict[str, str]]) -> BIDataset:
    ...
    def _create_detailed_metrics_dataset(self, reports: List[QualityReport], project_mapping: Optional[Dict[str, str]]) -> BIDataset:
    ...
    def _create_correlation_dataset(self, reports: List[QualityReport], project_mapping: Optional[Dict[str, str]]) -> BIDataset:
    ...
    def _create_anomaly_dataset(self, reports: List[QualityReport], project_mapping: Optional[Dict[str, str]]) -> BIDataset:
    ...
    def _create_predictive_dataset(self, reports: List[QualityReport], project_mapping: Optional[Dict[str, str]]) -> BIDataset:
    ...
    def _create_powerbi_template(self, datasets: Dict[str, BIDataset]) -> Dict[str, Any]:
