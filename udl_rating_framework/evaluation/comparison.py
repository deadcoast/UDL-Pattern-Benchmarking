...
class ComparisonEngine:
    ...
    def __init__(self, alpha: float = 0.05, bootstrap_samples: int = 1000):
        ...
        logger.info(f"Initialized ComparisonEngine with alpha={alpha}, bootstrap_samples={bootstrap_samples}")
    ...
    def compare_udls(self, reports: List[QualityReport]) -> ComparisonSummary:
        ...
        return ComparisonSummary(
            ...
            significant_comparisons=sum(1 for r in pairwise_results if r.is_significant),
            mean_effect_size=np.mean([r.cohens_d for r in pairwise_results]),
            score_statistics=summary_stats
        )
    
    def _compute_pairwise_comparisons(self, reports: List[QualityReport]) -> List[ComparisonResult]:
        """
        ...
        for i in range(len(reports)):
            for j in range(i + 1, len(reports)):
                ...
                ttest_stat, ttest_p = self._perform_ttest(score1, score2, report1, report2)
                wilcoxon_stat, wilcoxon_p = self._perform_wilcoxon_test(score1, score2, report1, report2)
                ...
                logger.debug(f"Compared {report1.udl_file} vs {report2.udl_file}: "
                           f"Δ={difference:.4f}, p={ttest_p:.4f}, d={cohens_d:.4f}")
        
        return results
    
    def _perform_ttest(self, score1: float, score2: float, 
                      report1: QualityReport, report2: QualityReport) -> Tuple[float, float]:
        """
    ...
    def _perform_wilcoxon_test(self, score1: float, score2: float,
                              report1: QualityReport, report2: QualityReport) -> Tuple[Optional[float], Optional[float]]:
        """
        ...
        try:
            ...
            statistic, p_value = mannwhitneyu(metrics1, metrics2, alternative='two-sided')
    ...
    def _compute_cohens_d(self, score1: float, score2: float,
                         report1: QualityReport, report2: QualityReport) -> float:
        """
    ...
    def _compute_rankings_with_confidence(self, reports: List[QualityReport]) -> List[RankingResult]:
        """
        ...
        for i, report in enumerate(reports):
            ...
            if len(metric_values) < 2:
                ...
                for _ in range(self.bootstrap_samples):
                    ...
                    resampled_metrics = np.random.choice(metric_values, size=len(metric_values), replace=True)
    ...
    def _compute_summary_statistics(self, reports: List[QualityReport], 
                                  pairwise_results: List[ComparisonResult]) -> Dict[str, float]:
        """
        ...
        return {
            ...
            "mean_absolute_difference": float(np.mean([abs(r.difference) for r in pairwise_results])),
            "max_absolute_difference": float(np.max([abs(r.difference) for r in pairwise_results])) if pairwise_results else 0.0,
        }