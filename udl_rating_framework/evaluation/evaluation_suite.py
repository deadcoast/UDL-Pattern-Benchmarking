...
class EvaluationSuite:
    ...
    def __init__(self, k_folds: int = 5, bootstrap_samples: int = 1000, confidence_level: float = 0.95):
        """
    ...
    def k_fold_cross_validation(self, 
                               ...
                               y: np.ndarray, 
                               model_fn: Callable,
                               metric_fn: Callable = None) -> Tuple[List[float], float, float]:
        """
    ...
    def compute_correlations_with_ci(self, 
                                   y_true: np.ndarray, 
                                   y_pred: np.ndarray) -> Tuple[float, Tuple[float, float], float, Tuple[float, float]]:
        """
    ...
    def bootstrap_confidence_intervals(self, 
                                     y_true: np.ndarray, 
                                     y_pred: np.ndarray,
                                     metrics: Dict[str, Callable] = None) -> Dict[str, Tuple[float, float]]:
        """
        ...
        if metrics is None:
            metrics = {
                ...
                'R2': lambda y_t, y_p: 1 - np.sum((y_t - y_p) ** 2) / np.sum((y_t - np.mean(y_t)) ** 2)
            }
    ...
    def evaluate(self, 
                ...
                model_fn: Optional[Callable] = None) -> EvaluationResult:
        """
        ...
        pearson_r, pearson_ci, spearman_r, spearman_ci = self.compute_correlations_with_ci(y_true, y_pred)
        
        # Calibration error (if confidences provided)
        calibration_error = 0.0
        if confidences is not None:
            calibration_error = self.compute_calibration_error(y_true, y_pred, confidences)
        ...
        if X is not None and model_fn is not None:
            cv_scores, mean_cv, std_cv = self.k_fold_cross_validation(X, y_true, model_fn)
