...
class EnsemblePredictor(nn.Module):
    ...
    def forward(self, token_ids: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, Any]]:
        """
        ...
        if self.method == 'simple_average':
            ...
            weights = torch.tensor([member.weight for member in self.members], 
            ...
            stacked_preds = member_predictions.permute(1, 2, 0)  # [batch, 1, num_members]
            stacked_preds = stacked_preds.squeeze(1)  # [batch, num_members]
            ensemble_pred = self.meta_learner(stacked_preds).unsqueeze(1)
            ensemble_cert = torch.mean(member_certainties, dim=0)  # Simple average for certainty
    ...
    def predict_with_uncertainty(self, token_ids: torch.Tensor) -> Dict[str, np.ndarray]:
        """
        ...
        if metadata['method'] == 'stacking':
            ...
            if meta_learner_path.exists():
                ensemble.meta_learner.load_state_dict(
                    torch.load(meta_learner_path, map_location=device, weights_only=True)
                )
...
class CTMEnsembleTrainer:
    ...
    def train_synchronization_diverse_ensemble(self,
                                             train_dataloader: DataLoader,
                                             val_dataloader: DataLoader,
                                             num_members: int = 5,
                                             num_epochs: int = 50) -> List[EnsembleMember]:
        """
        ...
        """
        logger.info(f"Training CTM ensemble with synchronization diversity ({num_members} members)")
    ...
    def _create_ctm_diverse_config(self, 
                                  member_idx: int, 
                                  total_members: int,
                                  neuron_select_strategies: List[str],
                                  memory_lengths: List[int],
                                  iteration_counts: List[int],
                                  synch_dimensions: List[int]) -> Dict[str, Any]:
        """
        Create CTM-specific diverse configuration.
        ...
        config['neuron_select_type'] = neuron_select_strategies[member_idx % len(neuron_select_strategies)]
        
        # Vary memory length (affects neuron-level model behavior)
        config['memory_length'] = memory_lengths[(member_idx // len(neuron_select_strategies)) % len(memory_lengths)]
        
        # Vary iteration count (affects temporal dynamics)
        config['iterations'] = iteration_counts[(member_idx // (len(neuron_select_strategies) * len(memory_lengths))) % len(iteration_counts)]
        
        # Vary synchronization dimensions
        synch_dim = synch_dimensions[member_idx % len(synch_dimensions)]
        config['n_synch_out'] = synch_dim
        config['n_synch_action'] = max(8, synch_dim // 2)  # Action synch is typically smaller
        
        # Vary neuron-level model depth
        config['deep_nlms'] = member_idx % 2 == 0  # Alternate between deep and shallow NLMs
        ...
        logger.info(f"Member {member_idx}: {config['neuron_select_type']}, "
    ...
    def train_temporal_diverse_ensemble(self,
                                      train_dataloader: DataLoader,
                                      val_dataloader: DataLoader,
                                      num_members: int = 5,
                                      num_epochs: int = 50) -> List[EnsembleMember]:
        ...
        """
        logger.info(f"Training CTM ensemble with temporal diversity ({num_members} members)")
        ...
        for i in range(num_members):
            ...
            # Create and train model
            member = self._train_single_member(
                config, train_dataloader, val_dataloader, num_epochs, f"temporal_member_{i}"
            )
    ...
    def _create_temporal_diverse_config(self, member_idx: int, total_members: int) -> Dict[str, Any]:
        """
        Create temporal-focused diverse configuration.
    ...
    def _train_single_member(self,
                            config: Dict[str, Any],
                            train_dataloader: DataLoader,
                            val_dataloader: DataLoader,
                            num_epochs: int,
                            member_id: str) -> EnsembleMember:
        """
    ...
    def train_stacking_ensemble(self,
                               ...
                               train_dataloader: DataLoader,
                               val_dataloader: DataLoader,
                               meta_epochs: int = 20) -> EnsemblePredictor:
        """
        ...
        ensemble = EnsemblePredictor(base_members, method='stacking', device=self.device)
        ...
        for epoch in range(meta_epochs):
            ...
            for token_ids, udl_representations in train_dataloader:
                ...
                base_predictions = torch.stack(base_predictions, dim=0)  # [num_members, batch, 1]
                base_predictions = base_predictions.permute(1, 2, 0).squeeze(1)  # [batch, num_members]
            ...
            logger.info(f"Meta-learner epoch {epoch+1}/{meta_epochs}: Loss={avg_loss:.4f}")
    ...
    def create_ensemble(self,
                       members: List[EnsembleMember],
                       method: str = 'weighted_average') -> EnsemblePredictor:
        """
...
def create_bootstrap_ensemble(model_factory: callable,
                             train_dataloader: DataLoader,
                             val_dataloader: DataLoader,
                             num_members: int = 5,
