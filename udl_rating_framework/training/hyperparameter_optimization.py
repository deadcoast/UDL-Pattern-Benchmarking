...
class CTMHyperparameterSpace:
    ...
    neuron_select_type: Any = field(
        default_factory=lambda: ["first-last", "random", "random-pairing"]
    )
    n_random_pairing_self: Any = field(
        default_factory=lambda: [0, 1, 2, 3, 4]
    )  # For random-pairing

    # Neuron-level model parameters (CTM-specific)
    memory_length: Any = field(
        default_factory=lambda: [3, 5, 10, 15, 20]
    )  # NLM history length
    deep_nlms: Any = field(
        default_factory=lambda: [True, False]
    )  # Deep vs shallow NLMs
    memory_hidden_dims: Any = field(
        default_factory=lambda: [64, 128, 256]
    )  # NLM hidden dimensions
    do_layernorm_nlm: Any = field(
...
class CTMHyperparameterOptimizer:
    ...
    ) -> Tuple[UDLRatingCTM, TrainingPipeline]:
        """
    ...
    def grid_search(
        ...
        for i, combination in enumerate(all_combinations):
            ...
            score = self._evaluate_hyperparameters(
                params, train_dataloader, val_dataloader, num_epochs
            )
            ...
            logger.info(
                f"Grid search progress: {i + 1}/{len(all_combinations)}, "
        ...
        logger.info(
            f"Grid search completed in {optimization_time:.2f}s. "
    ...
    def random_search(
        ...
        for trial in range(num_trials):
            ...
            logger.info(
                f"Random search progress: {trial + 1}/{num_trials}, "
                f"Current best score: {best_score:.4f}"
            )
        ...
        logger.info(
            f"Random search completed in {optimization_time:.2f}s. "
            f"Best score: {best_score:.4f}"
        )
        ...
        **kwargs,
        ...
        if method == "grid":
            return self.grid_search(
                train_dataloader, val_dataloader, num_epochs, num_trials
            )
        elif method == "random":
            return self.random_search(
                train_dataloader, val_dataloader, num_trials, num_epochs
            )
...
def create_focused_ctm_param_space(
    focus: str = "synchronization",
...
def optimize_ctm_hyperparameters(
    vocab_size: int,
    metrics: List[QualityMetric],
    aggregator: MetricAggregator,
    train_dataloader: DataLoader,
