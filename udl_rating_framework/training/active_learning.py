...
class ActiveLearningConfig:
    ...
    # Query strategy
    query_strategy: str = (
        "uncertainty_sampling"  # 'uncertainty_sampling', 'diversity_sampling', 'hybrid'
    )
    ...
    def select_samples(
        ...
        with torch.no_grad():
            for idx in unlabeled_indices:
                ...
                predictions, certainties, synch_out, tracking_data = model(
                    token_ids, track=self.use_temporal_analysis
                )
        ...
        logger.info(
            f"Selected {len(selected_indices)} samples using CTM {self.method} uncertainty sampling"
        )
...
class DiversitySampling(QueryStrategy):
    ...
    def select_samples(
        ...
        if self.method == "kmeans":
            ...
            return self._core_set_sampling(
                features, unlabeled_indices, labeled_indices, n_samples
            )
        else:
            raise ValueError(f"Unknown diversity method: {self.method}")

    def _extract_features(
        self, model: UDLRatingCTM, dataset: Dataset, indices: List[int]
    ) -> np.ndarray:
        """
    ...
    def _kmeans_sampling(
        self, features: np.ndarray, indices: List[int], n_samples: int
    ) -> List[int]:
        """
        ...
        for cluster_id in range(n_clusters):
            ...
            cluster_indices = [
                indices[i] for i in range(len(indices)) if cluster_mask[i]
            ]
    ...
    def _core_set_sampling(
        ...
        if not labeled_indices:
            ...
            return random.sample(
                unlabeled_indices, min(n_samples, len(unlabeled_indices))
            )
        ...
        logger.info(
            f"Selected {len(selected_indices)} samples using core-set diversity sampling"
        )
        return selected_indices
...
class HybridSampling(QueryStrategy):
    ...
    def select_samples(
        ...
        logger.info(
            f"Selected {len(selected_indices)} samples using hybrid sampling "
...
class ActiveLearner:
    ...
    def active_learning_iteration(self) -> Dict[str, Any]:
        ...
        """
        logger.info(
            f"Starting active learning iteration {len(self.learning_history) + 1}"
        )
        ...
        logger.info(
            f"Completed iteration {iteration_result['iteration']}: "
    ...
    def run_active_learning(self) -> List[Dict[str, Any]]:
        ...
        """
        logger.info(
            f"Starting active learning with {self.config.max_iterations} iterations"
        )
        ...
        logger.info(
            f"Completed active learning with {len(self.learning_history)} iterations"
        )
