...
class UDLDataset(Dataset):
    ...
    def _tokenize_udl(self, udl: UDLRepresentation) -> torch.Tensor:
        ...
        if len(indices) > self.max_length:
            ...
            indices.extend([self.vocab.token_to_index('<PAD>')] * (self.max_length - len(indices)))
...
class TrainingPipeline:
    ...
    def compute_loss(self, 
                     ...
                     ground_truth: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        ...
        confidence_scores = torch.softmax(certainties, dim=1)[:, 1]  # Take "confident" class
        ...
        prediction_accuracy = (torch.abs(predictions.squeeze() - ground_truth.squeeze()) < threshold).float()
    ...
    def train_epoch(self, dataloader: DataLoader) -> Dict[str, float]:
        ...
        for batch_idx, (token_ids, udl_representations) in enumerate(dataloader):
            ...
            ground_truth = torch.tensor(ground_truth_scores, dtype=torch.float32, device=self.device).unsqueeze(1)
            
            # Forward pass
            predictions, certainties, _ = self.model(token_ids)
            
            # Compute loss
            loss, rating_loss, confidence_loss = self.compute_loss(predictions, certainties, ground_truth)
    ...
    def train(self, 
        ...
        for epoch in range(num_epochs):
            ...
            self.training_history['train_rating_loss'].append(train_metrics['rating_loss'])
            self.training_history['train_confidence_loss'].append(train_metrics['confidence_loss'])
            
            # Validation
            if val_dataloader is not None:
                val_metrics = self.evaluate(val_dataloader)
                self.training_history['val_loss'].append(val_metrics['loss'])
                self.training_history['val_rating_loss'].append(val_metrics['rating_loss'])
                self.training_history['val_confidence_loss'].append(val_metrics['confidence_loss'])
                self.training_history['val_mae'].append(val_metrics['mae'])
                self.training_history['val_rmse'].append(val_metrics['rmse'])
                self.training_history['val_correlation'].append(val_metrics['correlation'])
    ...
    def get_training_summary(self) -> Dict[str, Any]:
        ...
        if self.training_history['val_loss']:
            summary.update({
                ...
                'best_val_correlation': max(self.training_history['val_correlation'])
            })
