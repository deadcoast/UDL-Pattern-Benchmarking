...
@click.option('--include-traces', is_flag=True,
              help='Include computation traces in output')
...
@click.pass_context
def rate_command(ctx: click.Context,
                 input_path: Path,
    ...
    try:
        ...
        logger.info(f"Metric weights: consistency={consistency_weight}, "
        ...
        for udl_file in udl_files:
            try:
                ...
                logger.info(f"Completed: {udl_file} (score: {report.overall_score:.4f})")
        ...
        if format == 'json':
            ...
            report_data = report_generator.generate_csv_report(reports, include_trace=include_traces)
        elif format == 'html':
