...
def evaluate_command(ctx: click.Context,
    ...
    try:
        ...
        if bootstrap_samples < 1000:
            raise click.BadParameter(f"bootstrap_samples must be at least 1000, got {bootstrap_samples}")
        
        if not 0 < confidence_level < 1:
            raise click.BadParameter(f"confidence_level must be between 0 and 1, got {confidence_level}")
        ...
        logger.info(f"Evaluation parameters: k_folds={k_folds}, bootstrap_samples={bootstrap_samples}")
        ...
        if 'metrics' in config:
            ...
            weights_dict.update({
                ...
                'structural_coherence': metrics_config.get('structural_coherence_weight', 0.25)
            })
            ...
            with torch.no_grad():
                for udl_repr in udl_representations:
                    ...
                    if len(token_ids) > max_length:
                        ...
                        token_ids.extend([vocab.get_token_id('<PAD>')] * (max_length - len(token_ids)))
                    ...
                    confidences.append(float(torch.softmax(certainty, dim=-1).max().cpu().item()))
            ...
            for udl_repr in udl_representations:
                ...
                for metric in metrics:
                    try:
                        ...
                        logger.warning(f"Failed to compute {metric.__class__.__name__} for {udl_repr.file_path}: {e}")
        ...
        if include_detailed_analysis:
            ...
            output_data['detailed_analysis'] = {
                ...
                'correlation_strength': 'strong' if evaluation_result.pearson_correlation > 0.7 else 
                                      'moderate' if evaluation_result.pearson_correlation > 0.5 else 'weak'
            }
        ...
        logger.info(f"Pearson correlation: {evaluation_result.pearson_correlation:.4f} "
                   f"(95% CI: {evaluation_result.pearson_ci[0]:.4f}-{evaluation_result.pearson_ci[1]:.4f})")
        logger.info(f"Spearman correlation: {evaluation_result.spearman_correlation:.4f} "
        ...
        logger.info(f"Cross-validation score: {evaluation_result.mean_cv_score:.4f} ± {evaluation_result.std_cv_score:.4f}")
