...
@click.option('--repo-path', type=click.Path(exists=True, path_type=Path),
              default=Path.cwd(), help='Git repository path')
...
@click.option('--platform', type=click.Choice(['github', 'jenkins', 'gitlab', 'azure']),
              multiple=True, default=['github'], help='CI/CD platform(s)')
@click.option('--output-dir', type=click.Path(path_type=Path), default=Path.cwd(),
...
def generate_cicd_workflows(platform: List[str], output_dir: Path, threshold: float,
                          timeout: int, retention_days: int):
...
@click.option('--workflow-dir', type=click.Path(exists=True, path_type=Path),
              default=Path.cwd(), help='Directory containing workflow files')
...
@click.option('--debounce', type=float, default=0.5,
              help='Debounce delay in seconds')
@click.option('--log-level', type=click.Choice(['DEBUG', 'INFO', 'WARNING', 'ERROR']),
              default='INFO', help='Log level')
def start_lsp_server(threshold: float, real_time: bool, debounce: float, log_level: str):
...
@click.option('--include-patterns', multiple=True,
...
def batch_process_files(input_path: Path, output: Path, format: str, workers: Optional[int],
                       chunk_size: int, timeout: float, cache_dir: Optional[Path],
                       no_cache: bool, include_patterns: List[str], 
                       exclude_patterns: List[str], streaming: bool):
    ...
    try:
        ...
        if input_path.is_dir():
            ...
            if streaming:
                ...
                for file_result in processor.process_files_streaming(file_paths, output):
...
@click.option('--plugin-type', type=click.Choice(['vscode', 'intellij', 'vim', 'emacs', 'sublime']),
              required=True, help='Type of plugin to generate')
@click.option('--output-dir', type=click.Path(path_type=Path), default=Path.cwd(),
...
@ide.command('install')
@click.option('--plugin-type', type=click.Choice(['vscode', 'intellij', 'vim']),
              required=True, help='Type of plugin to install')
@click.option('--plugin-dir', type=click.Path(exists=True, path_type=Path),
              required=True, help='Directory containing plugin files')
