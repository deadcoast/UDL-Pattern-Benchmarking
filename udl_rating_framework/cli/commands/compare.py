...
@click.option('--significance-level', type=float, default=0.05,
              help='Significance level for statistical tests')
@click.option('--include-effect-sizes', is_flag=True,
              help='Include effect size computations')
@click.option('--ranking', is_flag=True,
              help='Generate ranking with confidence intervals')
@click.option('--consistency-weight', type=float, default=0.25,
              help='Weight for consistency metric')
@click.option('--completeness-weight', type=float, default=0.25,
              help='Weight for completeness metric')
@click.option('--expressiveness-weight', type=float, default=0.25,
              help='Weight for expressiveness metric')
@click.option('--structural-coherence-weight', type=float, default=0.25,
              help='Weight for structural coherence metric')
@click.pass_context
def compare_command(ctx: click.Context,
                    input_paths: tuple,
    ...
    try:
        ...
        if not 0 < significance_level < 1:
            raise click.BadParameter(f"significance_level must be between 0 and 1, got {significance_level}")
        
        weights = [consistency_weight, completeness_weight,
                  expressiveness_weight, structural_coherence_weight]
        if abs(sum(weights) - 1.0) > 1e-6:
            raise click.BadParameter(f"Metric weights must sum to 1.0, got {sum(weights)}")
        ...
        if 'metrics' in config:
            ...
            consistency_weight = metrics_config.get('consistency_weight', consistency_weight)
            completeness_weight = metrics_config.get('completeness_weight', completeness_weight)
            expressiveness_weight = metrics_config.get('expressiveness_weight', expressiveness_weight)
            structural_coherence_weight = metrics_config.get('structural_coherence_weight', structural_coherence_weight)
        ...
        weights_dict = {
            ...
            metric_name_mapping['StructuralCoherenceMetric']: structural_coherence_weight
        }
        ...
        summary_stats = {
            ...
            'std_score': float((sum((s - sum(scores)/len(scores))**2 for s in scores) / len(scores))**0.5),
        ...
        logger.info(f"Average score: {summary_stats['mean_score']:.4f} ± {summary_stats['std_score']:.4f}")
        logger.info(f"Score range: {summary_stats['min_score']:.4f} - {summary_stats['max_score']:.4f}")
