...
@click.option('--early-stopping-patience', type=int, default=10,
              help='Early stopping patience')
@click.option('--alpha', type=float, default=0.7,
              help='Weight for rating loss in combined loss')
@click.option('--beta', type=float, default=0.3,
              help='Weight for confidence loss in combined loss')
@click.option('--d-model', type=int, default=256,
              help='CTM model dimension')
@click.option('--d-input', type=int, default=64,
              help='Input embedding dimension')
@click.option('--iterations', type=int, default=20,
              help='Number of CTM iterations')
...
@click.pass_context
def train_command(ctx: click.Context,
                  data_path: Path,
                  output_dir: Path,
    ...
    try:
        ...
        if not 0 < validation_split < 1:
            raise click.BadParameter(f"validation_split must be between 0 and 1, got {validation_split}")
        
        if abs(alpha + beta - 1.0) > 1e-6:
            raise click.BadParameter(f"alpha + beta must sum to 1.0, got {alpha + beta}")
        ...
        if 'training' in config:
            ...
            early_stopping_patience = training_config.get('early_stopping_patience', early_stopping_patience)
        ...
        logger.info(f"Model parameters: d_model={d_model}, d_input={d_input}, iterations={iterations}")
        logger.info(f"Training parameters: batch_size={batch_size}, lr={learning_rate}, epochs={epochs}")
