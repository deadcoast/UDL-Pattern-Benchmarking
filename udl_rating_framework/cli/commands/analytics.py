...
from udl_rating_framework.analytics import (
    TimeSeriesAnalyzer, PortfolioAnalyzer, TrendPredictor,
    ImprovementAdvisor, BusinessIntelligenceExporter
)
...
def load_reports_from_directory(reports_dir: Path) -> List[QualityReport]:
    ...
    for json_file in reports_dir.glob('*.json'):
        try:
            ...
            click.echo(f"Warning: Could not load report from {json_file}: {e}", err=True)
            continue
...
@analytics.command()
@click.option('--reports-dir', '-r', type=click.Path(exists=True, path_type=Path),
              required=True, help='Directory containing quality report JSON files')
@click.option('--udl-file', '-f', required=True, 
...
@click.option('--min-observations', default=10, 
              help='Minimum number of observations required')
def timeseries(reports_dir: Path, udl_file: str, metrics: tuple, 
               output: Optional[Path], min_observations: int):
    ...
    if len(file_reports) < min_observations:
        click.echo(f"Error: Insufficient data for {udl_file}. "
                  f"Found {len(file_reports)} reports, need at least {min_observations}.", err=True)
        return
    
    click.echo(f"Analyzing time series for {udl_file} with {len(file_reports)} reports...")
    ...
    try:
        if metrics:
            ...
            for metric in metrics:
                try:
                    evolution = analyzer.analyze_quality_evolution(reports, udl_file, metric)
...
@click.option('--reports-dir', '-r', type=click.Path(exists=True, path_type=Path),
              required=True, help='Directory containing quality report JSON files')
@click.option('--project-mapping', '-p', type=click.Path(exists=True, path_type=Path),
              help='JSON file mapping UDL files to projects')
@click.option('--output', '-o', type=click.Path(path_type=Path),
              help='Output file for the portfolio analysis report')
@click.option('--min-reports', default=5,
              help='Minimum reports per project for inclusion')
@click.option('--clustering-method', type=click.Choice(['kmeans', 'hierarchical']),
              default='kmeans', help='Clustering method to use')
@click.option('--n-clusters', default=3, help='Number of clusters for analysis')
def portfolio(reports_dir: Path, project_mapping: Optional[Path], output: Optional[Path],
              min_reports: int, clustering_method: str, n_clusters: int):
...
@click.option('--reports-dir', '-r', type=click.Path(exists=True, path_type=Path),
              required=True, help='Directory containing quality report JSON files')
@click.option('--udl-file', '-f', required=True,
              help='UDL file to predict trends for')
@click.option('--metrics', '-m', multiple=True,
...
@click.option('--models', multiple=True, type=click.Choice(['linear', 'polynomial', 'random_forest']),
              help='Models to use for prediction (default: all)')
@click.option('--output', '-o', type=click.Path(path_type=Path),
              help='Output file for the prediction report')
def predict(reports_dir: Path, udl_file: str, metrics: tuple, horizon: int,
            models: tuple, output: Optional[Path]):
    ...
    if len(file_reports) < 10:
        click.echo(f"Error: Insufficient data for {udl_file}. "
                  f"Found {len(file_reports)} reports, need at least 10.", err=True)
        return
    
    click.echo(f"Predicting trends for {udl_file} with {len(file_reports)} reports...")
    
    predictor_models = list(models) if models else ['linear', 'polynomial', 'random_forest']
    ...
    try:
        # Predict trends
        prediction_metrics = list(metrics) if metrics else ['overall_score', 'confidence']
        predictions = predictor.predict_quality_trends(reports, udl_file, prediction_metrics)
        ...
        for metric, result in predictions.items():
            current_value = result.predictions.iloc[0] if len(result.predictions) > 0 else 0.0
            future_value = result.predictions.iloc[-1] if len(result.predictions) > 0 else 0.0
            trend = "improving" if future_value > current_value else "declining" if future_value < current_value else "stable"
            
            click.echo(f"- {metric}: {trend} (R² = {result.model_performance.get('r2', 0.0):.3f})")
...
@click.option('--reports-dir', '-r', type=click.Path(exists=True, path_type=Path),
              required=True, help='Directory containing quality report JSON files')
@click.option('--udl-file', '-f', required=True,
              help='UDL file to generate improvement plan for')
@click.option('--target-score', type=float,
              help='Target quality score (default: current + 0.2)')
@click.option('--output', '-o', type=click.Path(path_type=Path),
              help='Output file for the improvement plan')
def improve(reports_dir: Path, udl_file: str, target_score: Optional[float], 
    ...
    click.echo(f"Generating improvement plan for {udl_file} with {len(file_reports)} reports...")
...
@click.option('--format', '-f', type=click.Choice(['csv', 'json', 'excel', 'parquet', 'xml']),
              default='csv', help='Export format')
@click.option('--aggregation', type=click.Choice(['detailed', 'summary', 'executive']),
              default='detailed', help='Aggregation level')
@click.option('--time-grouping', type=click.Choice(['daily', 'weekly', 'monthly']),
              default='daily', help='Time grouping for summary aggregation')
@click.option('--include-trends/--no-trends', default=True,
              help='Include trend analysis in export')
@click.option('--include-forecasts/--no-forecasts', default=False,
              help='Include forecast data in export')
@click.option('--project-mapping', '-p', type=click.Path(exists=True, path_type=Path),
              help='JSON file mapping UDL files to projects')
def export(reports_dir: Path, output_dir: Path, format: str, aggregation: str,
           time_grouping: str, include_trends: bool, include_forecasts: bool,
    ...
    try:
        ...
        click.echo(f"- Columns: {len(dataset.data.columns) if not dataset.data.empty else 0}")
...
@click.option('--platform', type=click.Choice(['powerbi', 'tableau']),
              required=True, help='BI platform to create package for')
@click.option('--project-mapping', '-p', type=click.Path(exists=True, path_type=Path),
              help='JSON file mapping UDL files to projects')
def dashboard(reports_dir: Path, output_dir: Path, platform: str,
              project_mapping: Optional[Path]):
