    ...
    if 'training' in config_dict:
        ...
        if 'validation_split' in training:
            ...
            if not 0 < val_split < 1:
                raise ValueError(f"validation_split must be between 0 and 1, got {val_split}")
        
        if 'alpha' in training and 'beta' in training:
            alpha = training['alpha']
            beta = training['beta']
            if abs(alpha + beta - 1.0) > 1e-6:
                raise ValueError(f"Training loss weights alpha + beta must sum to 1.0, got {alpha + beta}")
    ...
    if 'evaluation' in config_dict:
        ...
        if 'bootstrap_samples' in evaluation:
            ...
            if bootstrap_samples < 1000:
                raise ValueError(f"bootstrap_samples must be at least 1000, got {bootstrap_samples}")
