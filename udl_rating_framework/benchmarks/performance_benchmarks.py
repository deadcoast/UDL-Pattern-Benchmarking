...
class PerformanceBenchmark:
    ...
    def benchmark_metric_computation(
        ...
        logger.info(f"Benchmarking metric computation for {len(metric_names)} metrics across {len(udl_sizes)} sizes")
            ...
            for udl_size in udl_sizes:
                ...
                for _ in range(iterations):
                    ...
                    try:
                        ...
                        logger.error(f"Error computing {metric_name} for size {udl_size}: {e}")
                        continue
                ...
                if times:
                    ...
                    logger.debug(f"{metric_name} (size {udl_size}): {avg_time:.4f}s avg, {throughput:.2f} ops/s")
    ...
    def benchmark_ctm_inference(
        ...
        logger.info(f"Benchmarking CTM inference across {len(sequence_lengths)} seq lengths, "
        ...
        for vocab_size in vocab_sizes:
            for batch_size in batch_sizes:
                for seq_len in sequence_lengths:
                    ...
                    avg_time = statistics.mean(times)
                    avg_memory = statistics.mean(memory_usages)
                    throughput = (batch_size * seq_len) / avg_time if avg_time > 0 else 0.0
                    ...
                    logger.debug(f"CTM (vocab={vocab_size}, batch={batch_size}, seq={seq_len}): "
    ...
    def benchmark_batch_processing(
        ...
        logger.info(f"Benchmarking batch processing across {len(file_counts)} file counts, "
            ...
            for worker_count in worker_counts:
                ...
                for _ in range(iterations):
                    ...
                    try:
                        ...
                        logger.error(f"Error in batch processing (files={file_count}, workers={worker_count}): {e}")
                        continue
                ...
                if times:
                    ...
                    result = BenchmarkResult(
                        ...
                        metadata={
                            ...
                            'std_time': statistics.stdev(times) if len(times) > 1 else 0.0,
                            'speedup': throughput / (file_count / max(times)) if worker_count == 1 else None
                        }
                    )
                    
                    suite.add_result(result)
                    
                    logger.debug(f"Batch processing (files={file_count}, workers={worker_count}): "
    ...
    def verify_complexity_bounds(
        ...
        metric_names = list(MetricRegistry.list_metrics().keys())[:2]  # Test first 2 metrics
        ...
        for metric_name in metric_names:
            try:
                ...
                complexity_met = self._check_complexity_growth(sizes, times, target_complexity)
                results[f"metric_{metric_name}"] = complexity_met
                
                logger.debug(f"Metric {metric_name} complexity check: {'PASS' if complexity_met else 'FAIL'}")
        ...
        for i in range(num_rules):
            if i == 0:
                ...
                if i % 5 == 0:
                    ...
                    # Sequence rule
                    lines.append(f"rule_{i} ::= 'prefix_{i}' rule_{max(0, i-2)} 'suffix_{i}'")
                else:
                    # Simple rule
    ...
    def _generate_performance_plots(self, suite: BenchmarkSuite, filename_prefix: str):
        ...
        for result in suite.results:
            key = result.metadata.get('metric_name') or result.metadata.get('vocab_size') or 'default'
...
def run_comprehensive_benchmarks(output_dir: Optional[Path] = None) -> Dict[str, BenchmarkSuite]:
    """
    ...
    for name, suite in results.items():
        summary = suite.get_summary()
        logger.info(f"  {name}: {summary.get('total_benchmarks', 0)} tests, "
                   f"avg time: {summary.get('avg_execution_time', 0):.4f}s, "
                   f"avg throughput: {summary.get('avg_throughput', 0):.2f} ops/s")
    
    logger.info(f"Complexity verification: {sum(complexity_results.values())}/{len(complexity_results)} passed")
