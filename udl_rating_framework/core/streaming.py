...
class MemoryMappedFileReader:
    ...
    def find_line_boundaries(self, start: int, size: int, line_ending: str = '\n') -> Tuple[int, int]:
        """
        ...
        if end < self.file_size:
            ...
            search_end = min(self.file_size, end + 1000)  # Search up to 1000 chars forward
...
class StreamingChunker:
    ...
    def chunk_file(self, file_path: Path) -> Iterator[StreamingChunk]:
        ...
        """
        logger.info(f"Chunking file: {file_path} (size: {file_path.stat().st_size} bytes)")
        ...
        with MemoryMappedFileReader(file_path, self.config.encoding) as reader:
            ...
            while current_offset < file_size:
                ...
                chunk_content = chunk_data.decode(self.config.encoding, errors='replace')
                ...
                if chunk_start > 0 and self.config.overlap_size > 0:
                    ...
                    overlap_data = reader.read_chunk(overlap_start, chunk_start - overlap_start)
                    overlap_before = overlap_data.decode(self.config.encoding, errors='replace')
                
                if chunk_end < file_size and self.config.overlap_size > 0:
                    # Read overlap after
                    overlap_data = reader.read_chunk(chunk_end, min(self.config.overlap_size, file_size - chunk_end))
                    overlap_after = overlap_data.decode(self.config.encoding, errors='replace')
    ...
    def chunk_content(self, content: str, file_path: str = "memory") -> Iterator[StreamingChunk]:
        """
...
class StreamingProcessor:
    ...
    def __init__(self, 
        ...
        self.metric_names = metric_names or ['consistency', 'completeness', 'expressiveness', 'structural_coherence']
    ...
    def process_file_streaming(self, file_path: Path) -> Iterator[StreamingResult]:
        ...
        logger.info(f"Streaming processing completed: {self.processed_chunks} chunks in {total_time:.3f}s")
        
        # Cache aggregated result if enabled
        if self.cache and chunk_results:
            aggregated_result = self._aggregate_chunk_results(chunk_results, str(file_path))
            if aggregated_result:
                self.cache.put_udl(file_path, aggregated_result)
    
    def process_content_streaming(self, 
                                content: str, 
                                file_path: str = "memory") -> Iterator[StreamingResult]:
        """
        ...
        total_time = time.time() - start_time
        logger.info(f"Content streaming processing completed: {self.processed_chunks} chunks in {total_time:.3f}s")
    ...
    def _process_chunk(self, chunk: StreamingChunk) -> StreamingResult:
        ...
        udl = UDLRepresentation(full_content, chunk.metadata.get('file_path', 'unknown'))
    ...
    def _aggregate_chunk_results(self, 
                               chunk_results: List[StreamingResult], 
                               file_path: str) -> Optional[QualityReport]:
        """
        ...
        """
        valid_results = [r for r in chunk_results if r.error is None and r.partial_results]
        ...
        for metric_name, scores_weights in all_metric_scores.items():
            ...
            final_metric_scores[metric_name] = weighted_sum / total_metric_weight if total_metric_weight > 0 else 0.0
...
class AsyncStreamingProcessor:
    ...
    def process_file_async(self, file_path: Path) -> Iterator[StreamingResult]:
        ...
        try:
            ...
            logger.info(f"Async streaming processing completed: {results_yielded} results")
            ...
            try:
                ...
                try:
                    error_result = StreamingResult(
                        ...
                        file_path=getattr(chunk, 'metadata', {}).get('file_path', 'unknown'),
...
def aggregate_streaming_results(results: List[StreamingResult]) -> Optional[QualityReport]:
    """
