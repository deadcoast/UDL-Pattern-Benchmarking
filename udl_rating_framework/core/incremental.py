...
class DependencyTracker:
    ...
    def _setup_default_dependencies(self) -> None:
        ...
        self.add_dependency('consistency', ['rule_definition', 'rule_reference', 'constraint'])
        
        # Completeness metric depends on construct definitions
        self.add_dependency('completeness', ['construct_definition', 'terminal', 'non_terminal'])
        
        # Expressiveness metric depends on grammar structure
        self.add_dependency('expressiveness', ['grammar_rule', 'production', 'alternation'])
        
        # Structural coherence depends on overall structure
        self.add_dependency('structural_coherence', ['grammar_rule', 'dependency', 'hierarchy'])
    ...
    def analyze_construct_changes(self, 
                                old_udl: Optional[UDLRepresentation],
                                new_udl: UDLRepresentation) -> Set[str]:
        """
    ...
    def _get_construct_type(self, construct_name: str, udl: UDLRepresentation) -> Optional[str]:
...
class IncrementalCache:
    ...
    def update_metric_result(self, file_path: Path, metric_name: str, result: float) -> None:
    ...
    def invalidate_file(self, file_path: Path, affected_metrics: Optional[Set[str]] = None) -> None:
        """
        ...
        with self._lock:
            ...
            if affected_metrics is None:
                ...
                keys_to_remove = [
                    key for key in self.metric_results.keys()
                    if key[0] == file_path_str
                ]
            else:
                # Invalidate specific metrics
                keys_to_remove = [
                    (file_path_str, metric) for metric in affected_metrics
            ...
            logger.debug(f"Invalidated {len(keys_to_remove)} cache entries for {file_path}")
    ...
    def _load_cache(self) -> None:
        ...
        try:
            ...
            logger.debug(f"Loaded incremental cache: {len(self.file_snapshots)} snapshots, "
...
class IncrementalProcessor:
    ...
    def process_file(self, file_path: Path) -> IncrementalResult:
        ...
        logger.debug(f"Incremental processing completed: {file_path} "
    ...
    def _detect_changes(self, 
                       ...
                       new_snapshot: FileSnapshot) -> ChangeSet:
    ...
    def _process_full(self, 
                     ...
                     change_set: ChangeSet) -> IncrementalResult:
    ...
    def _process_incremental(self, 
        ...
        changed_constructs = self.dependency_tracker.analyze_construct_changes(old_udl, new_udl)
        affected_metrics = self.dependency_tracker.get_affected_metrics(changed_constructs)
        ...
        aggregator = MetricAggregator({name: 1.0/len(all_metric_scores) for name in all_metric_scores})
        overall_score = aggregator.aggregate(all_metric_scores)
        
        # Calculate confidence (simplified)
        confidence = 0.8 if recomputed_metrics else 0.9  # Lower confidence for partial recomputation
        ...
        cache_hit_ratio = len(cached_metrics) / len(self.metric_names) if self.metric_names else 0.0
