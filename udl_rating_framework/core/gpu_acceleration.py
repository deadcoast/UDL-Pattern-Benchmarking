    ...
    def _detect_devices(self) -> None:
        ...
        if torch.cuda.is_available():
            ...
            for i in range(device_count):
                ...
                logger.info(f"  Device {i}: {device_name} ({memory_total / 1024**3:.1f} GB)")
    ...
    def get_best_device(self, memory_required: Optional[int] = None) -> torch.device:
        ...
        for device_info in self.available_devices:
            if device_info['type'] == 'cpu':
                ...
                suitable_devices.append(device_info)
            elif device_info['memory_total'] and device_info['memory_total'] >= memory_required:
                suitable_devices.append(device_info)
        ...
        if suitable_devices[0]['type'] == 'cuda':
            ...
            for device_info in suitable_devices:
                if device_info['type'] == 'cuda':
                    ...
                    free_memory = torch.cuda.get_device_properties(device_info['id']).total_memory
    ...
    def __init__(self, 
        ...
        for udl in udl_representations:
            ...
            if len(token_ids) > max_length:
                ...
                token_ids.extend([vocabulary.pad_token_id] * (max_length - len(token_ids)))
...
class GPUAcceleratedCTM:
    ...
    def predict_batch(self, 
                     udl_representations: List[UDLRepresentation],
                     vocabulary: UDLTokenVocabulary) -> List[Dict[str, float]]:
        """
            ...
            for batch in dataloader:
                ...
                for i in range(len(ratings)):
                    ...
                    prediction = {
                        ...
                        'confidence': float(certainties[i, 0]),  # Use first certainty component
    ...
    def predict_single(self, 
                      udl_representation: UDLRepresentation,
                      vocabulary: UDLTokenVocabulary) -> Dict[str, float]:
        """
    ...
    def benchmark_inference(self, 
                          ...
                          benchmark_iterations: int = 10) -> Dict[str, float]:
        """
        ...
        logger.info(f"Benchmark results: {throughput:.1f} UDLs/sec, {avg_time:.4f}s avg time")
...
class GPUAcceleratedProcessor:
    ...
    def __init__(self,
        ...
        """
        if not TORCH_AVAILABLE:
            raise RuntimeError("PyTorch not available. Install PyTorch for GPU acceleration.")
    ...
    def process_udls(self, 
                    udl_representations: List[UDLRepresentation],
                    vocabulary: Optional[UDLTokenVocabulary] = None) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(f"GPU processing completed in {processing_time:.3f}s "
    ...
    def benchmark_performance(self, 
                            ...
                            vocabulary: Optional[UDLTokenVocabulary] = None) -> Dict[str, float]:
        """
    ...
    for batch_size in batch_sizes:
        for seq_len in sequence_lengths:
            ...
            try:
                ...
                logger.info(f"Benchmark {config_name}: {benchmark_result['throughput_udls_per_second']:.1f} UDLs/sec")
