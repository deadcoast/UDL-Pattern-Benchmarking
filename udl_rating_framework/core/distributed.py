...
from udl_rating_framework.core.multiprocessing import ProcessingResult, BatchProcessingStats
...
class DistributedConfig:
    ...
    def __post_init__(self):
        if self.resources_per_worker is None:
            self.resources_per_worker = {'cpu': 1.0, 'memory': 1024 * 1024 * 1024}  # 1GB
...
class DistributedProcessor:
    ...
    def __init__(self, config: Optional[DistributedConfig] = None):
        ...
        if self.config.backend == 'auto':
            if RAY_AVAILABLE:
                ...
                raise RuntimeError("No distributed computing backend available. Install Ray or Dask.")
        ...
        if self.config.backend == 'dask' and not DASK_AVAILABLE:
            raise RuntimeError("Dask backend requested but not available. Install Dask.")
    ...
    def initialize(self) -> None:
        ...
        logger.info(f"Distributed backend {self.config.backend} initialized successfully")
    ...
    def process_files_distributed(
        ...
        logger.info(f"Distributed processing completed: {successful} successful, {failed} failed, "
...
def process_files_distributed(
    ...
    if show_progress:
        def progress_callback(completed: int, total: int):
            ...
            logger.info(f"Distributed processing: {completed}/{total} ({percentage:.1f}%)")
