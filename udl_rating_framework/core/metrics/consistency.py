...
from udl_rating_framework.core.representation import UDLRepresentation, GrammarRule, Constraint
...
class ConsistencyMetric(QualityMetric):
    ...
    def find_contradictions(self, rules: List[GrammarRule]) -> List[Tuple[GrammarRule, GrammarRule]]:
        """
    ...
    def _are_mutually_exclusive(self, rhs1: List[str], rhs2: List[str]) -> bool:
        ...
        if (len(rhs1) == 1 and len(rhs2) == 1 and 
            rhs1[0] != rhs2[0] and 
            self._is_terminal(rhs1[0]) and self._is_terminal(rhs2[0])):
            return True
        ...
        for term1 in rhs1_terminals:
            for term2 in rhs2_terminals:
                if (term1, term2) in contradictory_pairs or (term2, term1) in contradictory_pairs:
                    return True
    ...
    def _is_terminal(self, symbol: str) -> bool:
        ...
        """
        return (symbol.startswith("'") and symbol.endswith("'")) or \
               (symbol.startswith('"') and symbol.endswith('"')) or \
               symbol.isdigit()
    
    def _have_conflicting_constraints(self, rule1: GrammarRule, rule2: GrammarRule) -> bool:
        """
    ...
    def _constraints_conflict(self, constraint1: Constraint, constraint2: Constraint) -> bool:
        """
        ...
        if constraint1.type == constraint2.type:
            ...
            if constraint1.condition != constraint2.condition:
                ...
                if ('not' in constraint1.condition and 
                    constraint1.condition.replace('not ', '') == constraint2.condition):
                    return True
                if ('not' in constraint2.condition and 
                    constraint2.condition.replace('not ', '') == constraint1.condition):
                    return True
    ...
    def _have_semantic_conflicts(self, rule1: GrammarRule, rule2: GrammarRule) -> bool:
        ...
        if 'operator' in meta1 and 'operator' in meta2:
            ...
            if (rule1.lhs == rule2.lhs and op1 != op2 and 
                op1 in ['::=', ':=', '='] and op2 in ['::=', ':=', '=']):
                return True
