...
class Construct:
    ...
    def __init__(self, name: str, construct_type: str, metadata: Dict[str, Any] = None):
        self.name = name
        self.construct_type = construct_type  # e.g., 'production_rule', 'terminal', 'operator'
...
class CompletenessMetric(QualityMetric):
    ...
    def __init__(self):
        ...
        self.language_type_requirements = {
            'basic_grammar': {
                'production_rules', 'terminals', 'non_terminals'
            },
            'expression_language': {
                'production_rules', 'terminals', 'non_terminals', 
            ...
            'programming_language': {
                'production_rules', 'terminals', 'non_terminals',
                'operators', 'keywords', 'identifiers', 'literals',
                'statements', 'expressions', 'control_flow'
            },
            'markup_language': {
                'production_rules', 'terminals', 'non_terminals',
                'tags', 'attributes', 'content', 'nesting'
            },
            'configuration_language': {
                'production_rules', 'terminals', 'non_terminals',
                'key_value_pairs', 'sections', 'comments'
            }
        }
    ...
    def get_required_constructs(self, language_type: str) -> Set[str]:
        ...
        """
        return self.language_type_requirements.get(language_type, {'production_rules', 'terminals'})
    ...
    def _infer_language_type(self, udl: UDLRepresentation) -> str:
        ...
        programming_keywords = {'if', 'then', 'else', 'while', 'for', 'function', 'var', 'let', 'const'}
    ...
    def _has_precedence_patterns(self, rules: List) -> bool:
        ...
        found_indicators = sum(1 for indicator in precedence_indicators 
    ...
    def _extract_structural_constructs(self, udl: UDLRepresentation) -> Set[Construct]:
        ...
        if any(any(pattern in rule.lhs.lower() for pattern in statement_patterns) 
               for rule in rules):
        ...
        if any(any(pattern in rule.lhs.lower() for pattern in expression_patterns) 
               for rule in rules):
        ...
        tokens = udl.get_tokens()
        if any(any(pattern in token.text.lower() for pattern in control_patterns) 
               for token in tokens):
        ...
        nesting_indicators = ['<', '>', '{', '}', '(', ')']
        if any(any(indicator in token.text for indicator in nesting_indicators) 
               for token in tokens):
            constructs.add(Construct('nesting', 'nesting'))
        
        # Look for key-value patterns (for configuration languages)
        if any('=' in token.text and token.type == TokenType.OPERATOR for token in tokens):
            constructs.add(Construct('key_value_pairs', 'key_value_pairs'))
        
        # Look for section patterns
        section_patterns = ['[', ']', 'section', 'group']
        if any(any(pattern in token.text.lower() for pattern in section_patterns) 
               for token in tokens):
