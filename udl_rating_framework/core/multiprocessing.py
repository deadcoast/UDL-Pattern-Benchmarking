...
def _process_single_udl(
    args: Tuple[str, str, List[str], Optional[Dict[str, float]], bool],
) -> ProcessingResult:
    """
    ...
    try:
        ...
        pipeline = RatingPipeline(
            metric_names=metric_names, weights=weights, enable_tracing=enable_tracing
        )
        ...
        error_msg = (
            f"{error_type} processing {file_path}: {str(e)}\n{traceback.format_exc()}"
        )
        logger.error(error_msg)
...
class ParallelProcessor:
    ...
    def process_files_parallel(
        ...
        logger.info(
            f"Starting parallel processing of {total_files} files with {self.max_workers} workers"
        )
            ...
            for future in as_completed(future_to_index):
                ...
                try:
                    ...
                    if result.success:
                        ...
                        successful += 1
                        logger.debug(
                            f"Successfully processed {file_path} in {result.processing_time:.3f}s"
                        )
                    else:
                        # Create error report
    ...
    def compute_metrics_parallel(
        ...
        logger.info(
            f"Starting parallel metric computation: {len(udl_contents)} UDLs × {len(metric_names)} metrics = {total_tasks} tasks"
        )
            ...
            for future in as_completed(future_to_args):
                ...
                try:
                    ...
                    if result.success:
                        ...
                        logger.debug(
                            f"Computed {metric_name} for {file_path}: {result.result}"
                        )
                    else:
                        results[metric_name].append((file_path, result.error))
                        logger.warning(
                            f"Failed to compute {metric_name} for {file_path}: {result.error}"
                        )
