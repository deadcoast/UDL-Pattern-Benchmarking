...
class InputValidator:
    ...
    LANGUAGE_CONSTRUCTS = {
        ...
        'expressions', 'statements', 'declarations', 'types',
        'functions', 'variables', 'comments', 'whitespace'
    }
    ...
    def validate_file(self, file_path: Path) -> ValidationResult:
        ...
        logger.info(f"Validated {file_path}: {format_type.value} format, "
    ...
    def validate_content(self, content: str, format_hint: Optional[UDLFormat] = None) -> ValidationResult:
        """
    ...
    def _detect_format(self, content: str, file_path: Optional[Path] = None) -> UDLFormat:
        """
        ...
        ebnf_score = sum(1 for pattern in self.EBNF_PATTERNS 
                        if re.search(pattern, content))
        
        # Check for BNF patterns
        bnf_score = sum(1 for pattern in self.BNF_PATTERNS 
                       if re.search(pattern, content))
        
        # Check for grammar patterns
        grammar_score = sum(1 for pattern in self.GRAMMAR_PATTERNS 
        ...
        if ebnf_score > 0 and ebnf_score >= bnf_score:
            ...
            return UDLFormat.DSL if file_path.suffix.lower() == '.dsl' else UDLFormat.UDL
        else:
            return UDLFormat.TEXT
    
    def _validate_content(self, content: str, format_type: UDLFormat) -> Tuple[List[str], List[str]]:
        """
    ...
    def _validate_grammar_syntax(self, content: str, format_type: UDLFormat, 
            ...
            if '::=' in line:
                ...
                if format_type == UDLFormat.EBNF:
                    if not self._check_balanced_brackets(rhs):
                        errors.append(f"Line {i}: Unbalanced brackets in EBNF expression")
    
    def _validate_grammar_declaration(self, content: str, 
                                    errors: List[str], warnings: List[str]) -> None:
        """
