...
class BatchProcessor:
    ...
    def __init__(self, config: Optional[BatchConfig] = None):
        ...
        default_metrics = ['consistency', 'completeness', 'expressiveness', 'structural_coherence']
    ...
    def process_directory(self, 
                         ...
                         exclude_patterns: Optional[List[str]] = None) -> BatchResult:
        """
        ...
        files = self.file_discovery.discover_files(
            directory,
            extensions=[p.replace('*', '') for p in patterns]
        )
    ...
    def process_files(self, file_paths: List[Path]) -> BatchResult:
        ...
        logger.info(f"Processing {self.total_count} files with {self.config.max_workers} workers")
    ...
    def process_files_streaming(self, 
                              ...
                              output_file: Path) -> Iterator[Dict[str, Any]]:
        """
    ...
    def _process_tasks_parallel(self, tasks: List[ProcessingTask]) -> None:
        ...
        with ProcessPoolExecutor(max_workers=self.config.max_workers) as executor:
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    chunk_results = future.result(timeout=self.config.timeout_per_file * len(chunk))
    ...
    def _process_chunk_worker(self, tasks: List[ProcessingTask]) -> Dict[str, Dict[str, Any]]:
    ...
    def _process_single_task(self, task: ProcessingTask) -> Dict[str, Any]:
        ...
        try:
            ...
            # Handle retries
            if (self.config.error_handling == 'retry' and 
    ...
    def _process_task_with_pipeline(self, 
                                  task: ProcessingTask, 
                                  pipeline: RatingPipeline) -> Dict[str, Any]:
        ...
        if self.config.include_detailed_metrics:
            ...
            result['computation_trace'] = [asdict(step) for step in report.computation_trace]
    ...
    def _create_batch_result(self, processing_time: float) -> BatchResult:
        ...
        summary_stats = {
            ...
            'success_rate': len(successful_results) / self.total_count if self.total_count > 0 else 0,
            'error_rate': len(self.errors) / self.total_count if self.total_count > 0 else 0,
            'average_processing_time': processing_time / self.total_count if self.total_count > 0 else 0
        }
        ...
        if successful_results:
            summary_stats.update({
                ...
                'median_quality': sorted([r['overall_score'] for r in successful_results])[len(successful_results)//2]
            })
            ...
            with open(output_file, 'w', newline='') as f:
                ...
                for file_path, file_result in result.file_results.items():
                    if 'error' in file_result:
                        writer.writerow([file_path, '', '', f"Error: {file_result['error']}"])
                    else:
                        writer.writerow([
                            file_path,
    ...
    try:
        if args.input_path.is_dir():
            ...
            if args.streaming:
                ...
                for file_result in processor.process_files_streaming(file_paths, args.output):
