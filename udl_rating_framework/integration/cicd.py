...
class CICDIntegration:
    ...
    def generate_jenkins_pipeline(self, 
                                pipeline_name: str = 'UDL Quality Check') -> str:
        """
...
"""
    ...
    def generate_gitlab_ci(self) -> str:
        ...
        gitlab_ci = {
            ...
            'udl_quality_check': {
                ...
                'artifacts': {
                    'reports': {
                        'junit': 'udl-quality-report.xml' if self.config.report_format == 'xml' else None
                    },
    ...
    def create_workflow_files(self, 
                            target_dir: Path,
                            platforms: Optional[List[str]] = None) -> Dict[str, Path]:
        """
            ...
            if workflow_file.suffix in ['.yml', '.yaml']:
                ...
                for section in required_sections:
                    if section not in content:
                        logger.warning(f"Missing required section '{section}' in Jenkinsfile")
                        return False
...
def main():
    ...
    parser.add_argument('--platform', choices=['github', 'jenkins', 'gitlab', 'azure'],
                       action='append', help='CI/CD platform(s)')
