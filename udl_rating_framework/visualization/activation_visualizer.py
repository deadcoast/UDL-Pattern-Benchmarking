...
class ActivationVisualizer:
    ...
    def plot_activation_heatmap(self, 
        ...
        if activation_type == 'pre':
            activations = tracking_data.pre_activations[:, batch_idx, :]  # [iterations, neurons]
        elif activation_type == 'post':
            activations = tracking_data.post_activations[:, batch_idx, :]  # [iterations, neurons]
        else:
        ...
        # Create heatmap
        im = ax.imshow(activations.T, aspect='auto', cmap='RdBu_r', interpolation='nearest')
        
        # Customize plot
        ax.set_xlabel('Iteration')
        ax.set_ylabel('Neuron Index')
        ax.set_title(f'{activation_type.capitalize()} Activation Patterns (Batch {batch_idx})')
        ...
        ax.set_yticks(range(0, tracking_data.n_neurons, max(1, tracking_data.n_neurons // 10)))
    ...
    def plot_activation_time_series(self, 
                                   tracking_data: TrackingData,
        ...
        for neuron_idx in neuron_indices:
            if neuron_idx < tracking_data.n_neurons:
                ax.plot(iterations, activations[:, neuron_idx], 
                       label=f'Neuron {neuron_idx}', linewidth=2)
        
        ax.set_xlabel('Iteration')
        ax.set_ylabel('Activation Value')
        ax.set_title(f'{activation_type.capitalize()} Activation Time Series (Batch {batch_idx})')
        ax.legend()
    ...
    def plot_neuron_correlation_matrix(self, 
                                     tracking_data: TrackingData,
                                     activation_type: str = 'post',
                                     batch_idx: int = 0,
                                     save_path: Optional[str] = None) -> plt.Figure:
        ...
        ax.set_ylabel('Neuron Index')
        ax.set_title(f'{activation_type.capitalize()} Neuron Correlation Matrix (Batch {batch_idx})')
    ...
    def create_activation_summary_report(self, 
                                       tracking_data: TrackingData,
        ...
        figures['pre_timeseries'] = self.plot_activation_time_series(
            tracking_data, activation_type='pre',
        ...
        figures['post_timeseries'] = self.plot_activation_time_series(
            tracking_data, activation_type='post',
            save_path=f"{save_dir}/post_activation_timeseries.png" if save_dir else None
        )
        
        figures['pre_distribution'] = self.plot_activation_distribution(
            tracking_data, 'pre',
            save_path=f"{save_dir}/pre_activation_distribution.png" if save_dir else None
        )
        
        figures['post_distribution'] = self.plot_activation_distribution(
            tracking_data, 'post',
            save_path=f"{save_dir}/post_activation_distribution.png" if save_dir else None
        )
