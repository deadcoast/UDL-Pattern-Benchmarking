...
class PropertyTest(ABC):
    ...
    @abstractmethod
    def test_property(
        self, metric: QualityMetric, test_cases: List[UDLRepresentation]
...
class FormalVerifier:
    ...
    def verify_metric(
        ...
        for test in self.standard_tests:
            try:
                ...
                if result.passed:
                    self.logger.info(
                        f"✓ {metric_name}: {result.property_name} property verified"
                    )
                else:
                    self.logger.warning(
                        f"✗ {metric_name}: {result.property_name} property failed "
                        f"({len(result.failures)} failures)"
                    )
            except Exception as e:
                self.logger.error(
                    f"Error testing {result.property_name} for {metric_name}: {e}"
                )
                results.append(
                    PropertyVerificationResult(
        ...
        if metric_name in self.custom_tests:
            for test in self.custom_tests[metric_name]:
                try:
                    ...
                    if result.passed:
                        self.logger.info(
                            f"✓ {metric_name}: Custom {result.property_name} property verified"
                        )
                    else:
                        self.logger.warning(
    ...
    def verify_all_metrics(
        self, metrics: List[QualityMetric], test_cases: List[UDLRepresentation]
        ...
        # Log summary
        self.logger.info(
            f"Formal verification complete: {passed_properties}/{total_properties} properties passed"
        )
        if not overall_success:
            self.logger.warning(
                f"Verification failed: {failed_properties} properties failed"
            )
