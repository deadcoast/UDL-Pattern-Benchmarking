            ...
            for line in args_section.split('\n'):
                ...
                if match:
                    ...
                    if name not in param_names_seen:
                        params.append(DocstringParameter(
                            name=name,
...
class DocstringValidator:
    ...
    def _validate_function(
        ...
        if docstring:
            ...
            report.undocumented_apis.append(UndocumentedAPI(
                name=func.__name__,
                module=module_name,
...
def find_undocumented_public_apis(package_name: str = "udl_rating_framework") -> List[UndocumentedAPI]:
                    ...
                    if isinstance(node, ast.ClassDef):
                        ...
                        if not has_doc:
                            undocumented.append(UndocumentedAPI(
                                name=node.name,
                                module=str(filepath).replace('/', '.').replace('.py', ''),
                                element_type='class',
...
def validate_docstrings(package_name: str = "udl_rating_framework") -> DocstringValidationReport:
...
def main():
    ...
    if report.signature_mismatches:
        ...
        for mismatch in report.signature_mismatches[:20]:
            print(f"  {mismatch.module}.{mismatch.function_name}: "
